...
def evaluate(data):
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            return {
                "isPasswordPolicyConfigured": False,
                "policyCount": 0,
        ...
        # Diagnostic: find the maximum minimum length across all policies
        min_lengths = []
        for rule in rules:
            min_len = rule.get("MinimumLength", 0)
            if isinstance(min_len, (int, float)) and min_len > 0:
                min_lengths.append(int(min_len))

        strongest_min = max(min_lengths) if min_lengths else 0
