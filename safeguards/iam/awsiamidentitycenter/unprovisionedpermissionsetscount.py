# unprovisionedpermissionsetscount.py - AWS IAM Identity Center (sso-admin API, awsJson1_1)
#
...
def transform(input):
    """
    unprovisionedPermissionSetsCount = number of permission sets provisioned to no AWS account. None when the
    instance has no permission sets, any permission set's accounts could not be read, or on any error body.
    """
    key = "unprovisionedPermissionSetsCount"
    FAIL = None
    ...
    try:
        ...
        idle = [str(sets[i]) for i in range(len(sets)) if counts[i] == 0]
        return {key: len(idle), "reason": str(len(idle)) + " of " + str(len(sets)) + " permission sets are provisioned to no account",
                "unprovisioned": idle[:25]}
