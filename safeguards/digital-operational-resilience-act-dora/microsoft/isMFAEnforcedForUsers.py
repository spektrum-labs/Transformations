"""
Transformation: isMFAEnforcedForUsers
Vendor: Microsoft  |  Category: digital-operational-resilience-act-dora
Evaluates: Checks that at least one enabled Conditional Access policy enforces MFA for all users.
"""
import json
from datetime import datetime


def extract_input(input_data):
    if isinstance(input_data, dict) and "data" in input_data and "validation" in input_data:
        return input_data["data"], input_data["validation"]
    data = input_data
    if isinstance(data, dict):
        wrapper_keys = ["api_response", "response", "result", "apiResponse", "Output"]
        for i in range(3):
            unwrapped = False
            for key in wrapper_keys:
                if key in data and isinstance(data.get(key), dict):
                    data = data[key]
                    unwrapped = True
                    break
            if not unwrapped:
                break
    return data, {"status": "unknown", "errors": [], "warnings": ["Legacy input format"]}


def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
                    recommendations=None, input_summary=None, transformation_errors=None,
                    api_errors=None, additional_findings=None):
    if validation is None:
        validation = {"status": "unknown", "errors": [], "warnings": []}
    return {
        "transformedResponse": result,
        "additionalInfo": {
            "dataCollection": {"status": "error" if (api_errors or []) else "success", "errors": api_errors or []},
            "validation": {"status": validation.get("status", "unknown"), "errors": validation.get("errors", []), "warnings": validation.get("warnings", [])},
            "transformation": {"status": "error" if (transformation_errors or []) else "success", "errors": transformation_errors or [], "inputSummary": input_summary or {}},
            "evaluation": {"passReasons": pass_reasons or [], "failReasons": fail_reasons or [], "recommendations": recommendations or [], "additionalFindings": additional_findings or []},
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isMFAEnforcedForUsers", "vendor": "Microsoft", "category": "digital-operational-resilience-act-dora"}
        }
    }


def get_policies(data):
    if isinstance(data, list):
        return data
    if isinstance(data, dict):
        val = data.get("data", None)
        if isinstance(val, list):
            return val
    return []


def evaluate(data):
    try:
        policies = get_policies(data)
        if not policies:
            return {"isMFAEnforcedForUsers": False, "error": "No conditional access policies found", "totalPolicies": 0, "mfaPoliciesForAllUsers": 0}

        mfa_all_users_count = 0
        mfa_policy_names = []

        for policy in policies:
            state = policy.get("state", "")
            if state != "enabled":
                continue
            grant_controls = policy.get("grantControls", None)
            if not grant_controls:
                continue
            built_in = grant_controls.get("builtInControls", [])
            if not isinstance(built_in, list):
                built_in = []
            if "mfa" not in built_in:
                continue
            conditions = policy.get("conditions", {})
            users = conditions.get("users", {})
            include_users = users.get("includeUsers", [])
            include_groups = users.get("includeGroups", [])
            if "All" in include_users or "all" in include_users:
                mfa_all_users_count = mfa_all_users_count + 1
                mfa_policy_names.append(policy.get("displayName", "Unnamed Policy"))
            elif len(include_groups) > 0 and len(users.get("excludeUsers", [])) == 0:
                mfa_all_users_count = mfa_all_users_count + 1
                mfa_policy_names.append(policy.get("displayName", "Unnamed Policy") + " (group-based)")

        is_enforced = mfa_all_users_count > 0
        return {
            "isMFAEnforcedForUsers": is_enforced,
            "totalPolicies": len(policies),
            "mfaPoliciesForAllUsers": mfa_all_users_count,
            "mfaPolicyNames": ", ".join(mfa_policy_names)
        }
    except Exception as e:
        return {"isMFAEnforcedForUsers": False, "error": str(e)}


def transform(input):
    criteriaKey = "isMFAEnforcedForUsers"
    try:
        if isinstance(input, str):
            input = json.loads(input)
        elif isinstance(input, bytes):
            input = json.loads(input.decode("utf-8"))
        data, validation = extract_input(input)
        if validation.get("status") == "failed":
            return create_response(result={criteriaKey: False}, validation=validation, fail_reasons=["Input validation failed"])
        eval_result = evaluate(data)
        result_value = eval_result.get(criteriaKey, False)
        extra_fields = {k: v for k, v in eval_result.items() if k != criteriaKey and k != "error"}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if result_value:
            pass_reasons.append("At least one enabled Conditional Access policy enforces MFA for all users")
            for k in extra_fields:
                pass_reasons.append(k + ": " + str(extra_fields[k]))
        else:
            fail_reasons.append("No Conditional Access policy found that enforces MFA for all users")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Create an enabled Conditional Access policy requiring MFA and targeting all users in Microsoft Entra ID")
        result_dict = {criteriaKey: result_value}
        for k in extra_fields:
            result_dict[k] = extra_fields[k]
        return create_response(
            result=result_dict, validation=validation,
            pass_reasons=pass_reasons, fail_reasons=fail_reasons, recommendations=recommendations,
            input_summary={criteriaKey: result_value})
    except Exception as e:
        return create_response(
            result={criteriaKey: False}, validation={"status": "error", "errors": [], "warnings": []},
            transformation_errors=[str(e)], fail_reasons=["Transformation error: " + str(e)])
