...
Evaluates: Checks secure score control profiles for an active DKIM signing control.
"""
import json
...
DKIM_KEYWORDS = ["dkim", "dkimsigning", "domainkeysidentifiedmail"]


def is_dkim_profile(profile_id, title):
    pid = profile_id.lower()
    ptitle = title.lower()
    for kw in DKIM_KEYWORDS:
...
def evaluate(data):
    try:
        ...
        if not profiles:
            return {"isDKIMConfigured": False, "error": "No secure score control profiles found", "profilesChecked": 0}
        ...
        for profile in profiles:
            ...
            if is_dkim_profile(profile_id, title):
        ...
        return {
            ...
            "dkimProfilesFound": len(matching_profiles),
            "activeDKIMProfiles": ", ".join(active_profiles),
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("DKIM signing control is active in Microsoft Secure Score profiles")
            for k in extra_fields:
                pass_reasons.append(k + ": " + str(extra_fields[k]))
        else:
            fail_reasons.append("No active DKIM signing control profile found in Secure Score profiles")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Enable DKIM signing for all custom email domains in Microsoft 365 Defender > Email Authentication Settings")
