...
Evaluates: Returns true if the majority of enrolled managed devices are in a compliant state, indicating EPP is actively enabled.
"""
import json
...
def evaluate(data):
    try:
        ...
        total = len(devices)
        if total == 0:
            return {"isEPPEnabled": False, "error": "No managed devices found", "totalDevices": 0, "compliantDevices": 0, "compliancePercentage": 0}
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("Majority of enrolled devices report compliant status, indicating EPP is actively enabled")
            for k in extra_fields:
                pass_reasons.append(k + ": " + str(extra_fields[k]))
        else:
            fail_reasons.append("Fewer than 50% of enrolled devices are compliant; EPP may not be fully enabled")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Review non-compliant devices in Microsoft Intune and ensure Microsoft Defender is active on all managed endpoints")
