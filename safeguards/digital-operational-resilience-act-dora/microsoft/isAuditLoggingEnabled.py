...
Evaluates: Confirms audit logging is enabled by checking for non-empty directory audit log entries.
"""
import json
...
def evaluate(data):
    try:
        logs = get_logs(data)
        log_count = len(logs)
        is_enabled = log_count > 0

        most_recent = ""
        categories = []
        if logs:
            first = logs[0]
            most_recent = first.get("activityDateTime", "")
            for log in logs:
        ...
        return {
            ...
            "mostRecentLogDate": most_recent,
            "logCategories": ", ".join(categories)
        }
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("Directory audit logs are present, confirming audit logging is enabled")
            for k in extra_fields:
                pass_reasons.append(k + ": " + str(extra_fields[k]))
        else:
            fail_reasons.append("No audit log entries returned; audit logging may be disabled or not configured")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Ensure Microsoft Entra ID audit logging is enabled and the integration has AuditLog.Read.All permissions")
