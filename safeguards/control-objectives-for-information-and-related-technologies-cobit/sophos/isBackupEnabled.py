...
Evaluates: Whether a backup solution is deployed and enabled by checking each endpoint's
assignedProducts array for backup-related product codes (e.g. 'backup').
"""
import json
...
def endpoint_has_backup(endpoint_item):
...
def evaluate(data):
    try:
        ...
        is_enabled = backup_count > 0

        return {
            "isBackupEnabled": is_enabled,
            "totalEndpoints": total,
            "backupEnabledCount": backup_count
        }
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("Backup product is assigned on at least one managed endpoint")
            pass_reasons.append("Backup enabled on " + str(extra_fields.get("backupEnabledCount", 0)) + " of " + str(extra_fields.get("totalEndpoints", 0)) + " endpoints")
        else:
            fail_reasons.append("No endpoints have a backup product assigned in Sophos Central")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Assign a Sophos backup product to managed endpoints via Sophos Central")
            recommendations.append("Verify Sophos Backup licensing is active for the tenant")
