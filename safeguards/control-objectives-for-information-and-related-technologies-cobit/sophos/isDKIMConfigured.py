...
Evaluates: Whether DKIM (DomainKeys Identified Mail) signing is configured and enabled for
outbound email by inspecting the Sophos Email settings response.
"""
import json
...
def check_dkim_in_setting(setting):
    dkim = setting.get("dkim", {})
    if dkim:
        if dkim.get("enabled", False):
            return True
        if dkim.get("configured", False):
            return True
        signing_keys = dkim.get("signingKeys", [])
        if signing_keys:
            return True
        key_enabled = dkim.get("keyEnabled", False)
        if key_enabled:
            return True
    outbound = setting.get("outbound", {})
    if outbound:
        dkim_outbound = outbound.get("dkim", {})
        if dkim_outbound and dkim_outbound.get("enabled", False):
            return True
    return False
...
def evaluate(data):
    try:
        ...
        if not items and isinstance(data, dict):
            if check_dkim_in_setting(data):
                return {
                    "isDKIMConfigured": True,
                    "configuredDomainCount": 1
                }
            return {
                "isDKIMConfigured": False,
                "error": "No email settings items found in response",
                "configuredDomainCount": 0
            }

        configured_count = 0
        for setting in items:
            if check_dkim_in_setting(setting):
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("DKIM signing is configured and enabled in Sophos Email settings")
            pass_reasons.append("Configured domain count: " + str(extra_fields.get("configuredDomainCount", 0)))
        else:
            fail_reasons.append("DKIM is not configured in Sophos Email settings")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Configure DKIM signing keys in Sophos Email Security settings")
            recommendations.append("Publish DKIM DNS records for all sending domains")
