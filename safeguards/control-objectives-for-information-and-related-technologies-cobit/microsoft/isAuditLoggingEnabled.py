...
Evaluates: Returns true if at least one directoryAudit log entry is returned from the Microsoft
Entra audit log endpoint, confirming audit logging is active
"""
import json
...
def evaluate(data):
    try:
        ...
        logging_active = log_count > 0
        most_recent = None
        if log_count > 0:
            most_recent = entries[0].get("activityDateTime", None)
        return {
            "isAuditLoggingEnabled": logging_active,
            "auditLogEntriesFound": log_count,
            "mostRecentEntryTimestamp": most_recent
        }
...
def transform(input):
    ...
    try:
        ...
        if result_value:
            ...
            fail_reasons.append(criteriaKey + " check failed: No audit log entries returned")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Ensure Microsoft Entra ID audit logging is enabled and that the integration has AuditLog.Read.All permission granted")
        result_dict = {}
