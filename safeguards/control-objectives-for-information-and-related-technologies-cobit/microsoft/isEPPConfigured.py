...
Evaluates: Checks that managed devices are enrolled via an MDM/Intune management agent and
have a compliance state, confirming Microsoft Defender EPP is configured across endpoints
"""
import json
    ...
    try:
        ...
        if not devices:
            return {
                ...
                "reason": "No managed devices found",
                "totalDevices": 0,
                "mdmEnrolledDevices": 0
            }
        ...
        for device in devices:
            ...
            has_compliance = device.get("complianceState", "") != ""
            is_mdm = False
            for agent_type in mdm_agents:
                if agent_type.lower() == agent:
                    is_mdm = True
                    break
            if is_mdm and has_compliance:
                mdm_count = mdm_count + 1
        epp_configured = mdm_count > 0
        return {
            "isEPPConfigured": epp_configured,
            "totalDevices": total,
            "mdmEnrolledDevices": mdm_count
        }
...
def transform(input):
    ...
    try:
        ...
        if result_value:
            ...
            recommendations.append("Enroll devices in Microsoft Intune MDM and configure device compliance policies to ensure Microsoft Defender EPP is provisioned across all endpoints")
        result_dict = {}
