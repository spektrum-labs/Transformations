...
the BeyondTrust Password Safe Sessions API endpoint.
"""
import json
...
def evaluate(data):
    try:
        ...
        if isinstance(data, dict):
            ...
            blocked_keywords = ["unauthorized", "forbidden", "access denied", "license"]
            is_blocked = False
            for kw in blocked_keywords:
                if kw in error_msg:
                    is_blocked = True
                    break
            if error_msg and is_blocked:
        ...
        if isinstance(data, list):
            ...
            for s in data:
                if isinstance(s, dict):
                    if bool(s.get("VideoRecording", False)):
                        has_video = True
                        break
            return {
