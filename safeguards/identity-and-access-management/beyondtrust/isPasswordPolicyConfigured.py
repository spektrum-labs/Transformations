...
Evaluates: Whether at least one password complexity/rotation policy (PasswordRule) is defined.
"""
import json
...
def evaluate(data):
    try:
        if isinstance(data, dict):
            ...
            return {"isPasswordPolicyConfigured": None, "error": "required fields missing from API response: PasswordRules"}
        ...
        for rule in rules:
            ...
            if isinstance(min_len, (int, float)):
                if int(min_len) > strongest_min:
                    strongest_min = int(min_len)

        return {
            "isPasswordPolicyConfigured": True,
            "policyCount": count,
