...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isSCIMProvisioningEnabled": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    sync_raw = settings.get("directory_sync_enabled", MISSING)
    mode_raw = settings.get("sso_provisioning_mode", MISSING)

    if sync_raw is MISSING and mode_raw is MISSING:
        return create_response(
    ...
    sync_on = sync_raw is not MISSING and as_bool(sync_raw)
    mode = mode_raw if mode_raw is not MISSING else None
