"""
Transformation: isThirdPartyContentRestricted
Vendor: Anthropic  |  Category: Artificial Intelligence
Product: Claude
Evaluates: Ensures third-party interactive content cannot render inside Claude, reducing the prompt-injection and data-exfiltration surface.
...
METADATA = {
    "transformationId": "isThirdPartyContentRestricted",
    "vendor": "Anthropic",
...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isThirdPartyContentRestricted": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    raw = settings.get("third_party_interactive_content_enabled", MISSING)

    if raw is MISSING:
        return create_response(
            result={"isThirdPartyContentRestricted": False, "third_party_interactive_content_enabled": None, "settingReported": False},
            validation=validation,
            fail_reasons=[
                "The effective organization settings response did not include a "
                "'third_party_interactive_content_enabled' row, so third-party interactive content restriction cannot be proven. A setting this "
            ...
            recommendations=[
                ...
                "Disable third-party interactive content in claude.ai > Organization settings unless a business case requires it."
            ],
    ...
    if result:
        pass_reasons = ["Third-party interactive content is disabled, reducing the prompt-injection and exfiltration surface inside Claude."]
        fail_reasons = []
        recommendations = []
    else:
        pass_reasons = []
        fail_reasons = ["Third-party interactive content is enabled, so externally authored interactive content can render inside Claude."]
        recommendations = ["Disable third-party interactive content in claude.ai > Organization settings unless a business case requires it."]

    return create_response(
        result={"isThirdPartyContentRestricted": result, "third_party_interactive_content_enabled": enabled, "settingReported": True},
        validation=validation,
        pass_reasons=pass_reasons,
        fail_reasons=fail_reasons,
        recommendations=recommendations,
        input_summary={"settingsReported": settings_count, "third_party_interactive_content_enabled": enabled},
        metadata=METADATA,
    )
...
def transform(input):
    try:
        ...
        return create_response(
            result={"isThirdPartyContentRestricted": False},
