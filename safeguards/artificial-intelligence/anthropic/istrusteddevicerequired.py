"""
Transformation: isTrustedDeviceRequired
Vendor: Anthropic  |  Category: Artificial Intelligence
Product: Claude Code
Evaluates: Ensures agent surfaces run only on devices the organization has designated as trusted.
...
METADATA = {
    "transformationId": "isTrustedDeviceRequired",
    "vendor": "Anthropic",
...
FLAGS = ('claude_code_trusted_devices_required', 'cowork_trusted_devices_required')
...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isTrustedDeviceRequired": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    if not reported:
        return create_response(
            result={"isTrustedDeviceRequired": False, "reportedFlags": [], "absentFlags": absent},
            validation=validation,
            fail_reasons=[
                "None of the settings rows that govern trusted device enforcement (" + ", ".join(FLAGS) +
            ...
            recommendations=[
                "Confirm the organization's plan exposes these controls. Require trusted devices for Claude Code and Cowork in claude.ai > Organization settings."
            ],
            input_summary={"settingsReported": settings_count, "flagsReported": 0},
            metadata=METADATA,
        )

    offenders = sorted(f for f, v in reported.items() if v is not True)
    compliant = sorted(f for f, v in reported.items() if v is True)
    result = not offenders

    if result:
        pass_reasons = [
            "Every reported flag governing trusted device enforcement is enabled (" +
            ", ".join(compliant) + ")."
        ]
        fail_reasons = []
        recommendations = []
        ...
        fail_reasons = [
            ...
            " reported flags governing trusted device enforcement are disabled: " + ", ".join(offenders) + "."
        ]
        recommendations = ["Require trusted devices for Claude Code and Cowork in claude.ai > Organization settings."]
    ...
    return create_response(
        result={
            "isTrustedDeviceRequired": result,
...
def transform(input):
    try:
        ...
        return create_response(
            result={"isTrustedDeviceRequired": False},
