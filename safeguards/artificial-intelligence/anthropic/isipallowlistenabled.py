...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isIPAllowlistEnabled": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    enabled_raw = settings.get("ip_allowlist_enabled", MISSING)

    if enabled_raw is MISSING:
        return create_response(
    ...
    ranges = settings.get("ip_allowlist_ip_ranges", MISSING)
    if ranges is MISSING or not isinstance(ranges, list):
        ranges = []
    enabled = as_bool(enabled_raw)
