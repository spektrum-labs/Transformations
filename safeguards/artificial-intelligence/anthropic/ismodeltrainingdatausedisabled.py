"""
Transformation: isModelTrainingDataUseDisabled
Vendor: Anthropic  |  Category: Artificial Intelligence
Product: Claude
Evaluates: Ensures the organization's content is excluded from model-improvement data use.
...
METADATA = {
    "transformationId": "isModelTrainingDataUseDisabled",
    "vendor": "Anthropic",
...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isModelTrainingDataUseDisabled": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    raw = settings.get("frontier_data_use_enabled", MISSING)

    if raw is MISSING:
        return create_response(
            result={"isModelTrainingDataUseDisabled": False, "frontier_data_use_enabled": None, "settingReported": False},
            validation=validation,
            fail_reasons=[
                "The effective organization settings response did not include a "
                "'frontier_data_use_enabled' row, so exclusion from model-improvement data use cannot be proven. A setting this "
            ...
            recommendations=[
                ...
                "Turn off frontier data use in claude.ai > Organization settings > Data and privacy."
            ],
    ...
    if result:
        pass_reasons = ["Frontier data use is disabled, so the organization's content is excluded from model-improvement data use."]
        fail_reasons = []
        recommendations = []
    else:
        pass_reasons = []
        fail_reasons = ["Frontier data use is enabled, so the organization's content is enrolled in model-improvement data use."]
        recommendations = ["Turn off frontier data use in claude.ai > Organization settings > Data and privacy."]

    return create_response(
        result={"isModelTrainingDataUseDisabled": result, "frontier_data_use_enabled": enabled, "settingReported": True},
        validation=validation,
        pass_reasons=pass_reasons,
        fail_reasons=fail_reasons,
        recommendations=recommendations,
        input_summary={"settingsReported": settings_count, "frontier_data_use_enabled": enabled},
        metadata=METADATA,
    )
...
def transform(input):
    try:
        ...
        return create_response(
            result={"isModelTrainingDataUseDisabled": False},
