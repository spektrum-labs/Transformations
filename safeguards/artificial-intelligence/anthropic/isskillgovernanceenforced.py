...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isSkillGovernanceEnforced": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    skills_raw = settings.get("skills_enabled", MISSING)
    reported = {f: as_bool(settings[f]) for f in UNGOVERNED_FLAGS if f in settings}

    if skills_raw is MISSING and not reported:
        return create_response(
    ...
    skills_on = None if skills_raw is MISSING else as_bool(skills_raw)
