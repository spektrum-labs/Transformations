...
Product: Claude
Evaluates: Ensures sensitive content is redacted from the organization's retained records.
...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isContentRedactionEnabled": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    raw = settings.get("content_redaction_enabled", MISSING)

    if raw is MISSING:
        return create_response(
            result={"isContentRedactionEnabled": False, "content_redaction_enabled": None, "settingReported": False},
            validation=validation,
            fail_reasons=[
                "The effective organization settings response did not include a "
                "'content_redaction_enabled' row, so content redaction cannot be proven. A setting this "
            ...
            recommendations=[
                ...
                "Turn on content redaction in claude.ai > Organization settings > Data and privacy."
            ],
    ...
    if result:
        pass_reasons = ["Content redaction is enabled, so sensitive content is redacted from retained organization records."]
        fail_reasons = []
        recommendations = []
    else:
        pass_reasons = []
        fail_reasons = ["Content redaction is disabled, so retained records hold unredacted content."]
        recommendations = ["Turn on content redaction in claude.ai > Organization settings > Data and privacy."]

    return create_response(
        result={"isContentRedactionEnabled": result, "content_redaction_enabled": enabled, "settingReported": True},
        validation=validation,
        pass_reasons=pass_reasons,
        fail_reasons=fail_reasons,
        recommendations=recommendations,
        input_summary={"settingsReported": settings_count, "content_redaction_enabled": enabled},
        metadata=METADATA,
    )
...
def transform(input):
    try:
        ...
        return create_response(
            result={"isContentRedactionEnabled": False},
