"""
Transformation: isExtensionAllowlistEnforced
Vendor: Anthropic  |  Category: Artificial Intelligence
Product: Claude Code
Evaluates: Ensures desktop extensions are restricted to an approved allowlist rather than installable at will.
...
METADATA = {
    "transformationId": "isExtensionAllowlistEnforced",
    "vendor": "Anthropic",
...
def evaluate(input):
    ...
    if refusal:
        ...
        return create_response(
            result={"isExtensionAllowlistEnforced": False, "endpointReachable": False, "httpStatus": refusal_status},
    ...
    raw = settings.get("desktop_extension_allowlist_enabled", MISSING)

    if raw is MISSING:
        return create_response(
            result={"isExtensionAllowlistEnforced": False, "desktop_extension_allowlist_enabled": None, "settingReported": False},
            validation=validation,
            fail_reasons=[
                "The effective organization settings response did not include a "
                "'desktop_extension_allowlist_enabled' row, so desktop extension restriction cannot be proven. A setting this "
            ...
            recommendations=[
                ...
                "Enable the desktop extension allowlist in claude.ai > Organization settings and populate it with approved extensions."
            ],
    ...
    if result:
        pass_reasons = ["The desktop extension allowlist is enforced, so only approved extensions can be installed."]
        fail_reasons = []
        recommendations = []
    else:
        pass_reasons = []
        fail_reasons = ["The desktop extension allowlist is not enforced, so any desktop extension can be installed."]
        recommendations = ["Enable the desktop extension allowlist in claude.ai > Organization settings and populate it with approved extensions."]

    return create_response(
        result={"isExtensionAllowlistEnforced": result, "desktop_extension_allowlist_enabled": enabled, "settingReported": True},
        validation=validation,
        pass_reasons=pass_reasons,
        fail_reasons=fail_reasons,
        recommendations=recommendations,
        input_summary={"settingsReported": settings_count, "desktop_extension_allowlist_enabled": enabled},
        metadata=METADATA,
    )
...
def transform(input):
    try:
        ...
        return create_response(
            result={"isExtensionAllowlistEnforced": False},
