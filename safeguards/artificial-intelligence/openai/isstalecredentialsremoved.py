...
Evaluates: No Admin or project API key is stale: unused for 90 days, older than 365 days, or owned by someone who lost project access.
API Source: getCredentialInventory workflow (admin_api_keys, projects, projects/{id}/api_keys, all pages)
...
def stale_reason(key, now, project=False):
    created = as_int(key.get("created_at"))
    used = as_int(key.get("last_used_at"))
    if created is None:
        return "created_at unreadable"
    if project and key.get("owner_project_access") == "inactive":
        return "owner no longer has access to the project"
    if now - created > MAX_AGE_DAYS * 86400:
        return "older than " + str(MAX_AGE_DAYS) + " days"
    if used is None:
        if now - created > UNUSED_DAYS * 86400:
            return "never used in " + str(UNUSED_DAYS) + "+ days"
        return None
    if now - used > UNUSED_DAYS * 86400:
        return "unused for more than " + str(UNUSED_DAYS) + " days"
    return None


def evaluate(input):
    data, validation = extract_input(input)
    if not isinstance(data, dict) or "adminApiKeys" not in data:
        return refused_or_unrecognised(data, validation, "Credential inventory")
    admin, admin_ok = read_list(data.get("adminApiKeys"))
    projects, projects_ok = read_list(data)
    per_project = data.get("projectApiKeys")
    if admin is None or projects is None or not isinstance(per_project, list):
        return refused_or_unrecognised(data, validation, "Credential inventory")
    if not admin_ok or not projects_ok or len(per_project) != len(projects):
        return fail(validation, "The credential inventory is incomplete (a list has more pages, or project key lists "
                    "do not line up with projects: " + str(len(per_project)) + " for " + str(len(projects)) + ").",
                    None, {"projects": len(projects), "projectKeyLists": len(per_project)})
    now = now_ts()
    stale = []
    total = 0
    for k in admin:
        total = total + 1
        why = stale_reason(k, now)
        if why:
            stale.append("admin key " + str(k.get("name") or k.get("id")) + ": " + why)
    for i in range(len(projects)):
        keys, keys_ok = read_list(per_project[i])
        pname = str(projects[i].get("name") or projects[i].get("id"))
        if keys is None or not keys_ok:
            return fail(validation, "API keys for project " + pname + " were not fully read.", None,
                        {"projects": len(projects)})
        for k in keys:
            total = total + 1
            why = stale_reason(k, now, True)
            if why:
                stale.append("project " + pname + " key " + str(k.get("name") or k.get("id")) + ": " + why)
    summary = {"keys": total, "stale": len(stale), "adminKeys": len(admin), "projects": len(projects)}
    if not admin:
        return fail(validation, "No Admin API keys were returned, although an admin key made the call.", None, summary)
    if stale:
        return fail(validation, str(len(stale)) + " of " + str(total) + " API keys are stale: " + "; ".join(stale[:10]) + ".",
                    "Revoke keys unused for 90 days, rotate keys older than a year, and remove keys whose owner left the project.",
                    summary, {"staleKeys": len(stale), "keys": total})
    return create_response(result={KEY: True, "keys": total}, validation=validation,
                           pass_reasons=["All " + str(total) + " Admin and project API keys were used within " + str(UNUSED_DAYS) +
                                         " days, are younger than " + str(MAX_AGE_DAYS) + " days, and belong to owners with access."],
