...
Evaluates: Every organization Admin API key has an expiry (expires_at set; OpenAI caps it at one year).
API Source: listAdminApiKeys (GET /v1/organization/admin_api_keys, all pages)
...
KEY = "isCredentialExpirationEnforced"
...
def evaluate(input):
    ...
    if items is None:
        return refused_or_unrecognised(data, validation, "Admin API keys")
    if not complete:
        return fail(validation, "The admin key list is incomplete (has_more is not false).", None, {"keys": len(items)})
    if not items:
        # The call itself was made with an admin key, so an empty list is an unreadable list.
        return fail(validation, "GET /v1/organization/admin_api_keys returned no keys, although an admin key made the call.",
                    None, {"keys": 0})
    never = [k for k in items if as_int(k.get("expires_at")) is None]
    summary = {"keys": len(items), "withoutExpiry": len(never)}
    if never:
        names = ", ".join(str(k.get("name") or k.get("id")) for k in never[:10])
        return fail(validation, str(len(never)) + " of " + str(len(items)) + " Admin API keys never expire: " + names + ".",
                    "Replace them with keys created with an expiry (expires_in_seconds, up to one year), then delete the old keys.",
                    summary, {"keysWithoutExpiry": len(never), "keys": len(items)})
    return create_response(result={KEY: True, "keys": len(items)}, validation=validation,
                           pass_reasons=["All " + str(len(items)) + " Admin API keys carry an expiry date, so each must be "
                                         "rotated within at most a year."],
