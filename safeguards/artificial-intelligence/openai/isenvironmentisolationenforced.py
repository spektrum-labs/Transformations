"""
Transformation: isEnvironmentIsolationEnforced
Vendor: OpenAI  |  Category: Artificial Intelligence
Product: OpenAI API (API Platform)
Evaluates: Workloads are separated into at least two active projects, and every project's API keys belong to project service accounts, not people.
API Source: getCredentialInventory workflow (projects, projects/{id}/api_keys, all pages)
...
KEY = "isEnvironmentIsolationEnforced"
...
def evaluate(input):
    ...
    if projects is None or not isinstance(per_project, list):
        return refused_or_unrecognised(data, validation, "Credential inventory")
    if not projects_ok or len(per_project) != len(projects):
        return fail(validation, "The project list is incomplete or project key lists do not line up with projects.",
                    None, {"projects": len(projects)})
    active = []
    personal = []
    for i in range(len(projects)):
        p = projects[i]
        if p.get("archived_at") is not None or str(p.get("status") or "active").lower() == "archived":
            continue
        active.append(p)
        keys, keys_ok = read_list(per_project[i])
        pname = str(p.get("name") or p.get("id"))
        if keys is None or not keys_ok:
            return fail(validation, "API keys for project " + pname + " were not fully read.", None, {"projects": len(projects)})
        for k in keys:
            owner = k.get("owner") if isinstance(k.get("owner"), dict) else {}
            if owner.get("type") != "service_account":
                personal.append(pname + "/" + str(k.get("name") or k.get("id")))
    summary = {"activeProjects": len(active), "personalKeys": len(personal)}
    reasons = []
    if len(active) < 2:
        reasons.append("only " + str(len(active)) + " active project, so every workload shares one boundary")
    if personal:
        reasons.append(str(len(personal)) + " project API key(s) are owned by people rather than service accounts: " +
                       ", ".join(personal[:10]))
    if reasons:
        return fail(validation, "Project isolation not shown: " + "; ".join(reasons) + ".",
                    "Split workloads into separate projects and issue each one keys from a project service account.",
                    summary, {"activeProjects": len(active), "personalKeys": len(personal)})
    return create_response(result={KEY: True, "activeProjects": len(active)}, validation=validation,
                           pass_reasons=[str(len(active)) + " active projects, and every project API key is owned by a "
