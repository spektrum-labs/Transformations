"""
Transformation: isSpendLimitEnforced
Vendor: OpenAI  |  Category: Artificial Intelligence
Product: OpenAI API (API Platform)
Evaluates: A hard monthly spend limit is configured for the organization and OpenAI reports it as enforcing.
API Source: getSpendLimit (GET /v1/organization/spend_limit)
...
def evaluate(input):
    ...
    if not isinstance(data, dict) or detect_refusal(data) or "enforcement" not in data:
        return refused_or_unrecognised(data, validation, "Spend limit")
    enforcement = data.get("enforcement") if isinstance(data.get("enforcement"), dict) else {}
    status = enforcement.get("status")
    amount = as_int(data.get("threshold_amount"))
    summary = {"status": status, "thresholdCents": amount, "interval": data.get("interval")}
    if status != "enforcing":
    ...
    return create_response(result={KEY: True, "thresholdAmount": dollars, "currency": data.get("currency")},
                           ...
                           pass_reasons=["A hard spend limit of " + str(dollars) + " " + str(data.get("currency")) + " per " +
                                         str(data.get("interval")) + " is enforcing: API usage stops at the cap."],
