...
Evaluates: Organization Owner role is held by a limited subset of members (at most 2, or at most 25%).
...
MAX_RATIO = 0.25
SMALL_ORG_ALLOWANCE = 2  # two owners is a continuity floor, never a finding
...
def evaluate(input):
    ...
    if not complete:
        return fail(validation, "The user list is incomplete (has_more is not false), so the owner share cannot be computed.",
                    "Check the listOrganizationUsers pagination settings.", {"members": len(items)})
    total = len(items)
    if total == 0:
        return fail(validation, "GET /v1/organization/users returned no members.", None, {"members": 0})
    # OpenAI documents role as `owner` or `reader`. Anything other than reader counts as
    # privileged, so an unknown or missing role can never make the share look smaller.
    owners = [u for u in items if str(u.get("role") or "").lower() != "reader"]
    count = len(owners)
    if count == 0:
        return fail(validation, "No member holds the Owner role. Every organization has one, so the roster or role field is unreadable.",
                    None, {"members": total, "owners": 0})
    pct = round(count * 100.0 / total, 2)
    ok = count <= SMALL_ORG_ALLOWANCE or count <= total * MAX_RATIO
    summary = {"members": total, "owners": count, "ownerPercent": pct}
    line = str(count) + " of " + str(total) + " organization members hold the Owner role (" + str(pct) + "%)."
    if ok:
        return create_response(result={KEY: True, "owners": count, "members": total, "ownerPercent": pct},
                               validation=validation,
                               pass_reasons=[line + " Within the limit of 2 owners or 25% of members."],
                               input_summary=summary)
    return fail(validation, line + " Above the limit of 2 owners or 25% of members.",
                "Move members who do not administer the organization to the Reader role.", summary,
                {"owners": count, "members": total, "ownerPercent": pct})
