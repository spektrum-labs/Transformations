"""
Transformation: isMFAEnforcedForUsers / isMFAEnabled
Vendor: Generic IDP
...
def transform(input):
    ...
    try:
        ...
        if is_enforced:
            pass_reasons.append(f"MFA is enforced with {len(mfa_enrolled)} active enrollment(s)")
        else:
            fail_reasons.append("No active MFA enrollment found")
            recommendations.append("Enable MFA enrollment for all users")

        return create_response(
            result={criteriaKey: is_enforced, "isMFAEnabled": is_enforced},
            validation=validation,
