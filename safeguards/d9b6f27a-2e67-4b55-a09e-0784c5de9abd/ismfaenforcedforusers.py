...
def transform(input):
    ...
    try:
        ...
        if is_enforced:
            ...
            pass_reasons.append(f"MFA enforced with {len(mfa_enrolled)} authentication methods enabled: {', '.join(method_names)}")
        else:
            fail_reasons.append("No MFA authentication methods configured")
