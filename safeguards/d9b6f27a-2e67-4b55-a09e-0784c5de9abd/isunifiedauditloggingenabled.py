...
def transform(input):
    ...
    try:
        ...
        if is_enabled:
            pass_reasons.append(f"Unified audit logging is enabled with {len(value)} audit log entries found")
        else:
            fail_reasons.append("Unified audit logging not enabled or no recent entries")
