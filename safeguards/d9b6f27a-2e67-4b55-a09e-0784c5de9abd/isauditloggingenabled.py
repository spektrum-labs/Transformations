...
Evaluates if audit logging is enabled for the identity provider.
"""
    ...
    try:
        ...
        if validation.get("status") == "failed" or legacy_status in ["failed", "error"]:
            return create_response(
                result={criteriaKey: False, "loggingEnabled": False},
        ...
        is_enabled = data is not None

        if is_enabled:
            pass_reasons.append("Audit logging is enabled")
        else:
            fail_reasons.append("Audit logging is not enabled")
            recommendations.append("Enable audit logging in Azure AD to track identity events")
        ...
        return create_response(
            ...
            input_summary={"loggingEnabled": is_enabled}
        )
