def transform(input):
    ...
    try:
        ...
        if 'authenticationMethodConfigurations' in input:
            mfa_enrolled = [obj for obj in input['authenticationMethodConfigurations'] if 'state' in obj and str(obj['state']).lower() == "enabled"]
        else:
            mfa_enrolled = []

        # Filter to keep only auth types that are NOT FIDO or OTP
        otherAuthTypes = [auth_type for auth_type in mfa_enrolled if auth_type['id'].lower() not in ['fido2', 'microsoftauthenticator', 'softwareoath']]
