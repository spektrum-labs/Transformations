...
Evaluates: Whether immutable vault/WORM is enabled (state is Locked or Unlocked)
"""
import json
...
def evaluate(data):
    ...
    try:
        ...
        is_immutable = immutability.get('state') in ['Locked', 'Unlocked']
        return {"isBackupImmutable": is_immutable, "immutabilityState": immutability.get('state', 'NotConfigured')}
    except Exception as e:
        return {"isBackupImmutable": False, "error": str(e)}


def transform(input):
    """Checks if immutable vault/WORM is enabled."""
