...
Evidence: the getBackups workflow, key dbBackups = RDS DescribeDBInstanceAutomatedBackups.

Rule (fail closed): true when at least one automated backup is active and EVERY active one
  * has BackupRetentionPeriod >= 1 (RDS takes a daily snapshot in PreferredBackupWindow and ships
    transaction logs continuously while retention is on), and
  * has a RestoreWindow.LatestTime no older than 26 hours (proof the daily cycle is running now).
Retained backups of deleted instances are counted but not judged.

Does not see: RDS instances with backups turned off (they never appear in this API), AWS Backup
plan schedules, EC2/EBS, S3, DynamoDB or EFS. A vendor error or unreadable body is reported as a
data-collection error (unevaluated), never as a pass.
...
def transform(input):
    try:
        ...
        for backup in active:
            ...
            retention = to_int(backup.get("BackupRetentionPeriod"))
            latest = parse_time(dig(backup, ["RestoreWindow", "LatestTime"]))
            lag = None if latest is None else int((now - latest).total_seconds() // 3600)
            ok = retention >= 1 and lag is not None and lag <= MAX_LAG_HOURS
            if not ok:
                failing.append(name)
            findings.append({
                "metric": name,
                "value": ok,
                "reason": "retention %d days; latest restorable point %s hours ago; window %s" % (
                    retention, "unknown" if lag is None else lag, backup.get("PreferredBackupWindow"))
            })
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("All %d active RDS automated backups run daily (retention >= 1 day, latest point within %d hours)" % (len(active), MAX_LAG_HOURS))
        elif len(active) == 0:
            fail_reasons.append("No active RDS automated backup found (%d listed)" % len(backups))
            recommendations.append("Enable RDS automated backups (BackupRetentionPeriod >= 1) on every database")
        else:
            fail_reasons.append("Daily backup not proven for: %s" % ", ".join(failing))
            recommendations.append("Check BackupRetentionPeriod and backup status on the listed instances")
