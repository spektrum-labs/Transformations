"""
Transformation: isFirewallEnabled
Vendor: Zscaler ZIA
Category: SASE / Firewall

Evaluates if cloud firewall rules are configured and active in Zscaler ZIA.
Checks for the presence of firewall rules and their enabled status.
"""

import json
from datetime import datetime


def extract_input(input_data):
    if isinstance(input_data, dict) and "data" in input_data and "validation" in input_data:
        return input_data["data"], input_data["validation"]
    data = input_data
    if isinstance(data, dict):
        wrapper_keys = ["api_response", "response", "result", "apiResponse", "Output"]
        for _ in range(3):
            unwrapped = False
            for key in wrapper_keys:
                if key in data and isinstance(data.get(key), dict):
                    data = data[key]
                    unwrapped = True
                    break
            if not unwrapped:
                break
    return data, {"status": "unknown", "errors": [], "warnings": ["Legacy input format"]}


def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
                    recommendations=None, input_summary=None, transformation_errors=None, api_errors=None, additional_findings=None):
    if validation is None:
        validation = {"status": "unknown", "errors": [], "warnings": []}
    return {
        "transformedResponse": result,
        "additionalInfo": {
            "dataCollection": {
                "status": "error" if (api_errors or []) else "success",
                "errors": api_errors or []
            },
            "validation": {
                "status": validation.get("status", "unknown"),
                "errors": validation.get("errors", []),
                "warnings": validation.get("warnings", [])
            },
            "transformation": {
                "status": "error" if (transformation_errors or []) else "success",
                "errors": transformation_errors or [],
                "inputSummary": input_summary or {}
            },
            "evaluation": {
                "passReasons": pass_reasons or [],
                "failReasons": fail_reasons or [],
                "recommendations": recommendations or [],
                "additionalFindings": additional_findings or []
            },
            "metadata": {
                "evaluatedAt": datetime.utcnow().isoformat() + "Z",
                "schemaVersion": "1.0",
                "transformationId": "isFirewallEnabled",
                "vendor": "Zscaler ZIA",
                "category": "SASE"
            }
        }
    }


def transform(input):
    criteriaKey = "isFirewallEnabled"

    try:
        if isinstance(input, str):
            input = json.loads(input)
        elif isinstance(input, bytes):
            input = json.loads(input.decode("utf-8"))

        data, validation = extract_input(input)

        if validation.get("status") == "failed":
            return create_response(
                result={criteriaKey: False, "rulesCount": 0, "enabledRulesCount": 0},
                validation=validation,
                fail_reasons=["Input validation failed"]
            )

        pass_reasons = []
        fail_reasons = []
        recommendations = []

        isFirewallEnabled = False
        rules_count = 0
        enabled_rules_count = 0

        # Handle raw list input (API returns array of rules directly)
        firewall_rules = None
        if isinstance(data, list):
            firewall_rules = data
        elif isinstance(data, dict):
            firewall_rules = data.get('firewallRules', data.get('responseData', None))

            # Check for firewall policy settings
            if data.get('firewallEnabled') or data.get('cloudFirewallEnabled'):
                isFirewallEnabled = True

        # Process rules list (from raw input or dict key)
        if isinstance(firewall_rules, list):
            rules_count = len(firewall_rules)

            if rules_count > 0:
                isFirewallEnabled = True

                # Count enabled rules
                for rule in firewall_rules:
                    if not isinstance(rule, dict):
                        continue
                    state = rule.get('state', rule.get('status', '')).upper()

                    if state:
                        if state == 'ENABLED':
                            enabled_rules_count += 1
                    elif rule.get('enabled', True):
                        enabled_rules_count += 1

        if isFirewallEnabled:
            pass_reasons.append(f"Cloud firewall is enabled with {rules_count} rules ({enabled_rules_count} active)")
        else:
            fail_reasons.append("No cloud firewall rules are configured")
            recommendations.append("Configure cloud firewall rules in Zscaler ZIA for network protection")

        return create_response(
            result={criteriaKey: isFirewallEnabled, "rulesCount": rules_count, "enabledRulesCount": enabled_rules_count},
            validation=validation,
            pass_reasons=pass_reasons,
            fail_reasons=fail_reasons,
            recommendations=recommendations,
            input_summary={
                "totalRules": rules_count,
                "enabledRules": enabled_rules_count
            }
        )

    except Exception as e:
        return create_response(
            result={criteriaKey: False, "rulesCount": 0, "enabledRulesCount": 0},
            validation={"status": "error", "errors": [], "warnings": []},
            transformation_errors=[str(e)],
            fail_reasons=[f"Transformation error: {str(e)}"]
        )
