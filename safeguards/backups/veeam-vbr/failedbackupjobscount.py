"""
failedBackupJobsCount - Veeam Backup & Replication

Backup job sessions created in the last 7 days that finished Failed (GET /api/v1/sessions typeFilter=BackupJob).
...
def evaluate(input):
    ...
    if len(sessions) == 0:
        return respond(None, "No backup job session finished in the last 7 days, so the failure count is not proven")
    failed = [s for s in sessions if dig(s, "result.result") == "Failed"]
    return respond(len(failed), str(len(failed)) + " of " + str(len(sessions)) + " backup job sessions finished in the last 7 days failed",
                   {"finishedSessions": len(sessions), "failedSessions": [name_of(s) for s in failed][:25]})
