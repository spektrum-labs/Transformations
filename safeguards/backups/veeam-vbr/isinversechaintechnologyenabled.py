"""
isInverseChainTechnologyEnabled - Veeam Backup & Replication

Every enabled backup job uses backupModeType ReverseIncremental (GET /api/v1/jobs).
...
KEY = "isInverseChainTechnologyEnabled"
METHOD = "getJobs"
...
def evaluate(input):
    ...
    bad = [name_of(j) + " (" + str(dig(j, "storage.advancedSettings.backupModeType")) + ")" for j in jobs
           if dig(j, "storage.advancedSettings.backupModeType") != "ReverseIncremental"]
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs do not use reverse incremental", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs use reverse incremental")
