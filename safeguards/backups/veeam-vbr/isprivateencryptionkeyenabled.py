"""
isPrivateEncryptionKeyEnabled - Veeam Backup & Replication

Every enabled backup job is encrypted with a password-derived key (encryptionType ByUserPassword) (GET /api/v1/jobs).
...
KEY = "isPrivateEncryptionKeyEnabled"
METHOD = "getJobs"
...
def evaluate(input):
    ...
    bad = [name_of(j) for j in jobs if not (encrypted(j) and dig(j, "storage.advancedSettings.storageData.encryption.encryptionType") == "ByUserPassword")]
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs are not encrypted with a password-derived key", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs are encrypted with a password-derived key (ByUserPassword)")
