"""
localStorageUtilizationPercentage - Veeam Backup & Replication

Used / capacity x 100 across online local repositories (Windows, Linux, hardened Linux, SMB, NFS) (GET /api/v1/backupInfrastructure/repositories/states).

Method: getRepositoryStates
...
def evaluate(input):
    items, why = vbr_list(parse_body(input), "repository states (GET /api/v1/backupInfrastructure/repositories/states)")
    if why:
    ...
    n = 0
    for r in items:
        if r.get("type") not in ["WinLocal", "LinuxLocal", "LinuxHardened", "Smb", "Nfs"]:
            continue
        if r.get("isOnline") is not True:
            return respond(None, "Local repository " + name_of(r) + " is offline, so its usage is unknown")
        c = as_num(r.get("capacityGB"))
        u = as_num(r.get("usedSpaceGB"))
        if c is None or u is None or c <= 0:
            return respond(None, "Local repository " + name_of(r) + " has no usable capacityGB / usedSpaceGB")
        cap += c
        used += u
        n += 1
    if n == 0:
        return respond(None, "No local backup repository was found")
    pct = round(used * 100.0 / cap, 2)
    return respond(pct, str(round(used, 1)) + " of " + str(round(cap, 1)) + " GB used across " + str(n) + " local repositories", {"localRepositories": n})
