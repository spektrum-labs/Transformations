"""
isBackupImmutable - Veeam Backup & Replication

Every repository (or scale-out performance extent) that an enabled backup job writes to enforces immutability (GET jobs, repositories, scaleOutRepositories).
...
def evaluate(input):
    ...
    for j in ctx["jobs"]:
        ...
        for r in repos:
            if not immutable(r):
                bad.append(name_of(j) + " -> " + name_of(r) + " (" + str(r.get("type")) + ")")
    if bad:
        return respond(False, str(len(bad)) + " job targets do not enforce immutability", {"mutableTargets": bad[:25]})
    return respond(True, "Every repository written by the " + str(len(ctx["jobs"])) + " enabled backup jobs enforces immutability for at least 1 day")
