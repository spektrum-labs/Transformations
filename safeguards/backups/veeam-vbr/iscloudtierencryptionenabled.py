"""
isCloudTierEncryptionEnabled - Veeam Backup & Replication

Every scale-out repository with a capacity tier that an enabled job uses encrypts it (capacityTier.encryption or job encryption).
...
def evaluate(input):
    ...
    for j in ctx["jobs"]:
        ...
        if why:
            return respond(False, why)
        if sobr is None or dig(sobr, "capacityTier.isEnabled") is not True:
            continue
        sid = str(sobr.get("id")).lower()
        if sid not in tiers:
            tiers[sid] = {"sobr": sobr, "jobs": []}
        tiers[sid]["jobs"].append(j)
    if len(tiers) == 0:
        return respond(False, "No enabled backup job uses a scale-out repository with a capacity tier")
    bad = []
    for sid in tiers:
        t = tiers[sid]
        if dig(t["sobr"], "capacityTier.encryption.isEnabled") is True:
            continue
        if len([j for j in t["jobs"] if not encrypted(j)]) == 0:
            continue
        bad.append(name_of(t["sobr"]))
    if bad:
        return respond(False, str(len(bad)) + " capacity tiers in use are not encrypted by the tier or by every job", {"unencryptedCapacityTiers": bad})
    return respond(True, "All " + str(len(tiers)) + " capacity tiers in use are encrypted")
