"""
isRestorePointHashVerificationEnabled - Veeam Backup & Replication

Every enabled backup job has a scheduled storage-level health check (backupHealth) (GET /api/v1/jobs).
...
KEY = "isRestorePointHashVerificationEnabled"
METHOD = "getJobs"
...
def evaluate(input):
    ...
    bad = []
    for j in jobs:
        h = dig(j, "storage.advancedSettings.backupHealth")
        sched = dig(h, "weekly.isEnabled") is True or dig(h, "monthly.isEnabled") is True
        if not (dig(h, "isEnabled") is True and sched):
            bad.append(name_of(j))
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs have no scheduled health check", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs run a scheduled storage-level health check")
