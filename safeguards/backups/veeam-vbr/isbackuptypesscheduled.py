"""
isBackupTypesScheduled - Veeam Backup & Replication

Every enabled backup job runs automatically on a daily, monthly, periodic, continuous or chained schedule (GET /api/v1/jobs).
...
def evaluate(input):
    ...
    bad = []
    for j in jobs:
        s = j.get("schedule") if isinstance(j.get("schedule"), dict) else {}
        kinds = [k for k in ["daily", "monthly", "periodically", "continuously", "afterThisJob"] if dig(s, k + ".isEnabled") is True]
        if s.get("runAutomatically") is not True or len(kinds) == 0:
            bad.append(name_of(j))
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs have no automatic schedule", {"unscheduledJobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs run on an automatic schedule")
