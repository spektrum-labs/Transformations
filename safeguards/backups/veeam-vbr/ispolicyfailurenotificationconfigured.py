"""
isPolicyFailureNotificationConfigured - Veeam Backup & Replication

Every enabled backup job sends SNMP, or custom email notifications on error to at least one recipient (GET /api/v1/jobs).
...
def evaluate(input):
    jobs, why = enabled_jobs(parse_body(input), "jobs (GET /api/v1/jobs)")
    if why:
        return respond(False, why)
    if len(jobs) == 0:
        return respond(False, "No enabled backup job was found")

    bad = []
    for j in jobs:
        n = dig(j, "storage.advancedSettings.notifications")
        snmp = dig(n, "sendSNMPNotifications") is True
        mail = dig(n, "emailNotifications")
        rcpt = dig(mail, "recipients")
        custom = (dig(mail, "isEnabled") is True and isinstance(rcpt, list) and len(rcpt) > 0 and
                  dig(mail, "notificationType") == "UseCustomNotificationSettings" and
                  dig(mail, "customNotificationSettings.notifyOnError") is True)
        if not (snmp or custom):
            why = "global email settings (not readable with a Backup Viewer account)" if dig(mail, "notificationType") == "UseGlobalNotificationSettings" else "no SNMP or error email"
            bad.append(name_of(j) + ": " + why)
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs have no evidenced failure notification", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs notify on failure by SNMP or email")
