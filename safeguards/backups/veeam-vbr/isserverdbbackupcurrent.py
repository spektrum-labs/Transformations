"""
isServerDbBackupCurrent - Veeam Backup & Replication

A configuration backup session finished Success in the last 7 days (GET /api/v1/sessions typeFilter=ConfigurationBackup).
...
def evaluate(input):
    ...
    last = latest(ok)
    return respond(True, str(len(ok)) + " configuration backup sessions ended Success in the last 7 days; latest " + str(last.get("endTime")))
