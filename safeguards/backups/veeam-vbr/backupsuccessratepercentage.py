"""
backupSuccessRatePercentage - Veeam Backup & Replication

Success / finished backup job sessions in the last 7 days x 100 (GET /api/v1/sessions typeFilter=BackupJob).

Method: getBackupSessions
...
def evaluate(input):
    sessions, why = read_sessions(input, "BackupJob", 7, "backup job sessions (GET /api/v1/sessions)")
    if why:
        return respond(None, why)
    if len(sessions) == 0:
        return respond(None, "No backup job session finished in the last 7 days, so there is no success rate")
    ok = len([s for s in sessions if dig(s, "result.result") == "Success"])
    rate = round(ok * 100.0 / len(sessions), 2)
    return respond(rate, str(ok) + " of " + str(len(sessions)) + " backup job sessions finished in the last 7 days ended Success (warnings are not success)",
                   {"finishedSessions": len(sessions), "successfulSessions": ok})
