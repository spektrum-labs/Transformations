"""
isBackupTested - Veeam Backup & Replication

The latest finished SureBackup session in the last 90 days ended Success (GET /api/v1/sessions typeFilter=SureBackup).
...
def evaluate(input):
    ...
    if last is None:
        return respond(False, "No SureBackup session finished in the last 90 days")
    res = dig(last, "result.result")
    return respond(res == "Success", "Latest SureBackup session " + name_of(last) + " (" + str(last.get("endTime")) + ") ended " + str(res))
