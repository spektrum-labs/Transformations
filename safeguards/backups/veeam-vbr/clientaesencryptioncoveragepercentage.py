"""
clientAESEncryptionCoveragePercentage - Veeam Backup & Replication

Enabled backup jobs with job-level (source-side AES-256) encryption / all enabled backup jobs x 100 (GET /api/v1/jobs).
...
KEY = "clientAESEncryptionCoveragePercentage"
METHOD = "getJobs"
...
def evaluate(input):
    ...
    if len(jobs) == 0:
        return respond(None, "No enabled backup job was found")

    enc = len([j for j in jobs if encrypted(j)])
    pct = round(enc * 100.0 / len(jobs), 2)
    return respond(pct, str(enc) + " of " + str(len(jobs)) + " enabled backup jobs use job-level encryption (AES-256, applied at the source before data leaves the proxy)",
                   {"encryptedJobs": enc, "enabledBackupJobs": len(jobs)})
