"""
areBackupsTested - Veeam Backup & Replication

Result (Success/Warning/Failed) of the latest finished SureBackup session in the last 90 days (GET /api/v1/sessions typeFilter=SureBackup).

Method: getSureBackupSessions
...
def evaluate(input):
    sessions, why = read_sessions(input, "SureBackup", 90, "SureBackup sessions (GET /api/v1/sessions)")
    if why:
        return respond(None, why)
    last = latest(sessions)
    if last is None:
        return respond(None, "No SureBackup session finished in the last 90 days, so recoverability testing is not evidenced")
    res = dig(last, "result.result")
    return respond(res, "Latest SureBackup session " + name_of(last) + " (" + str(last.get("endTime")) + ") ended " + str(res),
                   {"sureBackupSessions90d": len(sessions)})
