"""
isForeverIncrementalBackupEnabled - Veeam Backup & Replication

Every enabled backup job uses forward incremental with no synthetic or active fulls (forever incremental) (GET /api/v1/jobs).
...
KEY = "isForeverIncrementalBackupEnabled"
METHOD = "getJobs"
...
def evaluate(input):
    ...
    bad = []
    for j in jobs:
        adv = dig(j, "storage.advancedSettings")
        mode = dig(adv, "backupModeType")
        syn = dig(adv, "synthenticFulls.isEnabled")
        act = dig(adv, "activeFulls.isEnabled")
        if not (mode == "Incremental" and syn is False and act is False):
            bad.append(name_of(j) + " (" + str(mode) + ", synthetic full " + str(syn) + ", active full " + str(act) + ")")
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs are not forever incremental", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs are forever forward incremental (no synthetic or active fulls)")
