"""
isBackupEnabled - Veeam Backup & Replication

At least one backup job is enabled (isDisabled false) and scheduled (schedule.runAutomatically) (GET /api/v1/jobs).
...
def evaluate(input):
    ...
    auto = [j for j in jobs if dig(j, "schedule.runAutomatically") is True]
    if len(auto) == 0:
        return respond(False, str(len(jobs)) + " enabled backup jobs, none scheduled to run automatically")
    return respond(True, str(len(auto)) + " enabled backup jobs run automatically", {"enabledBackupJobs": len(jobs)})
