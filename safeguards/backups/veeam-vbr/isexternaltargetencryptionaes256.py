"""
isExternalTargetEncryptionAES256 - Veeam Backup & Replication

Every enabled job that writes to object storage (directly, as a scale-out extent or capacity tier) is AES-256 encrypted by the job or the capacity tier.

Method: getRepositoryTargets
...
def evaluate(input):
    ...
    bad = []
    for j in ctx["jobs"]:
        repos, sobr, why = resolve(ctx, j)
        if why:
            return respond(False, why)
        direct = [r for r in repos if r.get("type") in OBJECT_TYPES]
        tier = sobr is not None and dig(sobr, "capacityTier.isEnabled") is True
        if len(direct) == 0 and not tier:
            continue
        external += 1
        if encrypted(j):
            continue
        if len(direct) == 0 and dig(sobr, "capacityTier.encryption.isEnabled") is True:
            continue
        bad.append(name_of(j))
    if external == 0:
        return respond(False, "No enabled backup job writes to an external (object storage) target, so external encryption is not evidenced")
    if bad:
        return respond(False, str(len(bad)) + " of " + str(external) + " jobs writing to object storage are not encrypted", {"jobs": bad[:25]})
    return respond(True, "All " + str(external) + " jobs writing to object storage are encrypted (Veeam uses 256-bit AES)")
