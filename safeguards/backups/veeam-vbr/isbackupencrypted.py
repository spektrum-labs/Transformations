"""
isBackupEncrypted - Veeam Backup & Replication

Every enabled backup job has storage encryption enabled (GET /api/v1/jobs storageData.encryption.isEnabled).
...
def evaluate(input):
    ...
    bad = [name_of(j) for j in jobs if not encrypted(j)]
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(jobs)) + " enabled backup jobs do not encrypt backup files", {"unencryptedJobs": bad[:25]})
    return respond(True, "All " + str(len(jobs)) + " enabled backup jobs encrypt backup files")
