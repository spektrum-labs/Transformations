...
Evaluates: True when every resource in the Druva Resource Status report has backupEnabled = Yes. Druva does not tag criticality, so this proves ALL registered resources, which includes the critical ones.
...
FAIL_VALUE = False
RECOMMENDATION = "Enable backup on every Druva backup set, or remove resources that are no longer protected."
...
def evaluate(records):
    disabled = [text(r.get("resourceName")) or text(r.get("backupset")) or "unnamed"
                for r in records if text(r.get("backupEnabled")).lower() != "yes"]
    summary = {"resourcesReported": len(records), "resourcesNotBackupEnabled": len(disabled),
               "resourcesNotBackupEnabledSample": disabled[:10]}
    if len(records) == 0:
        return False, summary, ["Druva Resource Status report returned no resources"]
    if len(disabled) == 0:
        return True, summary, ["All " + str(len(records)) + " Druva backup sets report backupEnabled = Yes"]
    return False, summary, [str(len(disabled)) + " of " + str(len(records)) + " Druva backup sets do not report backupEnabled = Yes"]
