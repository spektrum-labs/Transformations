...
Evaluates: Count of jobs in the Druva Backup Activity report (last 24 hours) whose status is not Successful. Only Successful is a documented status value, so every other status counts as failed. None when no jobs are reported.
...
def evaluate(records):
    ...
    summary = {"backupJobsReported": len(records), "nonSuccessfulStatuses": statuses}
    if len(records) == 0:
        return None, summary, ["Druva Backup Activity report returned no jobs in its window; count not proven"]
