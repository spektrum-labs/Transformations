# isbackuptypesscheduled.py - Commvault (Command Center REST API, webconsole/commandcenter api)
#
...
def transform(input):
    """
    isBackupTypesScheduled = true when at least one server plan protects something and every server plan
    with associated entities is ENABLED with a positive RPO (its backup schedule). false when a plan in use
    is disabled, backup-disabled or has no RPO, when no server plan is in use, or on an unreadable body.
    Laptop, Office 365 and other plan types are not judged.
    """
    ...
    try:
        ...
        bad = [pname(p) + " (status " + str(p.get("status")) + ", RPO " + str(p.get("RPO")) + ")" for p in used
               if str(p.get("status") or "").upper() != "ENABLED" or (as_int(p.get("RPO")) or 0) <= 0]
        if bad:
            return {key: False, "reason": str(len(bad)) + " of " + str(len(used)) + " server plans in use are not enabled with a schedule", "plans": bad[:25]}
        return {key: True, "reason": "All " + str(len(used)) + " server plans in use are enabled with a positive RPO"}
