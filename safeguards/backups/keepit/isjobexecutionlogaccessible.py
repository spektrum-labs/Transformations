# isjobexecutionlogaccessible.py - Keepit
#
...
def transform(input):
    ...
    <cancelled>) and its outcome (<state> or the end timestamp).
    ...
    try:
        ...
        for r in rows:
            found = False
            for j in r["backupJobs"]:
                ended = j.get("succeeded") or j.get("failed") or j.get("cancelled")
                if j.get("started") and ended and job_state(j) in ["successful", "unsuccessful", "incomplete", "cancelled"]:
                    found = True
                    break
            if found:
                with_log.append(r["connector"])
            else:
                without_log.append(r["connector"])
        result = len(without_log) == 0
        if result:
            reason = "All " + str(len(rows)) + " cloud connectors return finished backup jobs with start, end and outcome"
        else:
            reason = str(len(without_log)) + " of " + str(len(rows)) + " cloud connectors return no finished backup job with start, end and outcome in the last 7 days"
        return {
            key: result,
            "reason": reason,
            "connectorsWithJobLog": with_log,
