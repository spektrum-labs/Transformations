# backupsuccessratepercentage.py - Keepit
#
...
def transform(input):
    """
    Returns backupSuccessRatePercentage = successful / finished backup jobs x 100 (one
    decimal), over every cloud connector's /jobs window (the last 7 days). Finished means
    successful, unsuccessful, incomplete or cancelled; jobs still scheduled or running are
    left out. The requirement compares with greaterThan "95.0" (inclusive).

    Proves: the success rate Keepit's own job records show for every connector.
    Does not prove: anything outside the window. Returns None when the body is unreadable, a
    connector's /jobs was not read, or no backup job finished.
    """
    ...
    try:
        ...
        if len(rows) == 0:
            return {key: None, "reason": "The account has no cloud connectors, so there is no success rate"}
        terminal = sum([r["terminal"] for r in rows])
        if terminal == 0:
            return {key: None, "reason": "No backup job finished in the /jobs window on any connector; the rate is not proven", "connectorCount": len(rows)}
        ok = sum([r["counts"]["successful"] for r in rows])
        rate = round(100.0 * ok / terminal, 1)
        per = {}
        for r in rows:
            if r["terminal"] > 0:
                per[r["connector"]] = round(100.0 * r["counts"]["successful"] / r["terminal"], 1)
            else:
                per[r["connector"]] = None
        return {
            key: rate,
            "reason": str(ok) + " of " + str(terminal) + " finished backup jobs across " + str(len(rows)) + " connectors succeeded (" + str(rate) + "%)",
            "successfulJobs": ok,
            "finishedBackupJobs": terminal,
            "successRateByConnector": per,
        }
