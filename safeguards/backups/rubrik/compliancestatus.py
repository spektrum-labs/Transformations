"""
Transformation: complianceStatus
Vendor: Rubrik  |  Category: Backup  |  Product: Rubrik Security Cloud (RSC)
Evaluates: At least 80% of active protected objects are IN_COMPLIANCE with their SLA.
...
def evaluate(input):
    ...
    summary = {"inCompliance": c["inCompliance"], "outOfCompliance": c["outOfCompliance"]}
    if judged == 0:
        return fail(validation, "No active protected object has an SLA compliance verdict.", None, summary)
    pct = round(100.0 * c["inCompliance"] / judged, 2)
    extra = {"compliancePercentage": pct, "compliantCount": c["inCompliance"], "nonCompliantCount": c["outOfCompliance"]}
    if pct < THRESHOLD:
        return fail(validation, "Only " + str(pct) + "% of active protected objects are in SLA compliance (threshold " + str(THRESHOLD) + "%).",
                    "Investigate the out-of-compliance objects in RSC > Compliance.", summary, extra, value=False)
    return ok(validation, True, str(pct) + "% of active protected objects are in SLA compliance (threshold " + str(THRESHOLD) + "%).", summary, extra)
