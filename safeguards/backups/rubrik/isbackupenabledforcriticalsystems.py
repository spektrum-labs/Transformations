...
Evaluates: At least 80% of active objects are protected and none of the protected are out of compliance beyond 20%.
...
Note: Scope = protected + NoSla objects; objects an administrator set to Do Not Protect are reported, not counted.
...
KEY = "isBackupEnabledForCriticalSystems"
...
def evaluate(input):
    ...
    if failure:
        return failure
    in_scope = c["protectedObjects"] + c["noSla"]
    summary = {"protectedObjects": c["protectedObjects"], "noSla": c["noSla"], "doNotProtect": c["doNotProtect"]}
    if in_scope == 0:
        return fail(validation, "No active object is in backup scope (protected or awaiting an SLA).", None, summary, value=False)
    pct = round(100.0 * c["protectedObjects"] / in_scope, 2)
    extra = {"coveragePercentage": pct, "doNotProtectCount": c["doNotProtect"]}
    if pct < THRESHOLD:
        return fail(validation, "Only " + str(pct) + "% of in-scope active objects are protected (threshold " + str(THRESHOLD) + "%).",
                    "Assign SLA domains to the objects with no SLA.", summary, extra, value=False)
    return ok(validation, True, str(pct) + "% of in-scope active objects are protected by an SLA domain; " + str(c["doNotProtect"]) +
              " objects are explicitly excluded (Do Not Protect).", summary, extra)
