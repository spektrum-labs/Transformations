...
Evaluates: Every SLA domain protecting objects is retention-locked (compliance or governance).
...
def evaluate(input):
    ...
    if failure:
        return failure
    gaps = [str(s.get("name")) for s in slas if not (s.get("isRetentionLockedSla") is True or s.get("retentionLockMode") in ("COMPLIANCE", "GOVERNANCE"))]
    summary = {"slaDomainsInUse": len(slas), "notRetentionLocked": gaps}
    if gaps:
        return fail(validation, str(len(gaps)) + " of " + str(len(slas)) + " SLA domains in use are not retention-locked: " + ", ".join(gaps) + ".",
                    "Enable Retention Lock on these SLA domains.", summary)
    return ok(validation, True, "All " + str(len(slas)) + " SLA domains in use are retention-locked.", summary)
