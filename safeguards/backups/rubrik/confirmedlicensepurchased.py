...
Evaluates: The RSC account answers for this service account and protects at least one object.
...
Note: Evidence of a licence in use: RSC answers and protects objects. RSC exposes no licence entitlement to a read-only service account.
...
def evaluate(input):
    ...
    if c["protectedObjects"] < 1:
        return fail(validation, "The RSC account answered but protects no active object, so there is no evidence of a Rubrik subscription in use.",
                    None, summary, value=False)
    return ok(validation, True, "The RSC account answered for this service account and protects " + str(c["protectedObjects"]) + " active objects.", summary)
