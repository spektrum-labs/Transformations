...
Evaluates: Every Rubrik cluster reports encryption at rest.
...
def evaluate(input):
    ...
    if failure:
        return failure
    gaps = [str(c.get("name")) for c in nodes if c.get("isEncrypted") is not True]
    summary = {"clusters": len(nodes), "notEncrypted": gaps}
    if gaps:
        return fail(validation, "Rubrik cluster(s) not encrypted at rest: " + ", ".join(gaps) + ".", "Enable encryption at rest on these clusters.", summary)
    return ok(validation, True, "All " + str(len(nodes)) + " Rubrik clusters are encrypted at rest.", summary)
