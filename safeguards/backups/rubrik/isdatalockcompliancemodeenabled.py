"""
Transformation: isDataLockComplianceModeEnabled
Vendor: Rubrik  |  Category: Backup  |  Product: Rubrik Security Cloud (RSC)
Evaluates: Every SLA domain protecting objects is retention-locked in COMPLIANCE mode.
...
KEY = "isDataLockComplianceModeEnabled"
...
def evaluate(input):
    ...
    if failure:
        return failure
    gaps = [str(s.get("name")) + " (" + str(s.get("retentionLockMode")) + ")" for s in slas if s.get("retentionLockMode") != "COMPLIANCE"]
    summary = {"slaDomainsInUse": len(slas), "notComplianceLocked": gaps}
    if gaps:
        return fail(validation, str(len(gaps)) + " of " + str(len(slas)) + " SLA domains in use are not retention-locked in COMPLIANCE mode: " + ", ".join(gaps) + ".",
                    "Enable Retention Lock in Compliance mode on SLA domains that hold recovery copies.", summary)
    return ok(validation, True, "All " + str(len(slas)) + " SLA domains in use are retention-locked in COMPLIANCE mode.", summary)
