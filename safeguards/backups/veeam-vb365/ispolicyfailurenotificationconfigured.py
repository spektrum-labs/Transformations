"""
isPolicyFailureNotificationConfigured - Veeam Backup for Microsoft 365

Email notifications are enabled, notifyOnFailure is on, and a recipient is set (GET /v8/VboEmailSettings).
...
def evaluate(input):
    ...
    if "enableNotification" not in body:
        return respond(False, "Email settings response has no enableNotification field")
    to = body.get("to")
    if body.get("enableNotification") is not True:
        return respond(False, "Email notifications are disabled")
    if body.get("notifyOnFailure") is not True:
        return respond(False, "Email notifications are on but notifyOnFailure is off")
    if not isinstance(to, str) or to.strip() == "":
        return respond(False, "No notification recipient is set")
    return respond(True, "Failure emails are sent to " + str(len([x for x in to.split(";") if x.strip()])) + " recipient(s)")
