"""
localStorageUtilizationPercentage - Veeam Backup for Microsoft 365

(capacity - free) / capacity x 100 across JET-based (local) backup repositories (GET /v8/BackupRepositories).
...
def evaluate(input):
    ...
    n = 0
    for r in allrepos:
        if isinstance(r.get("objectStorage"), dict):
            continue
        c = as_num(r.get("capacityBytes"))
        f = as_num(r.get("freeSpaceBytes"))
        if c is None or f is None or c <= 0 or f > c:
            return respond(None, "Local repository " + name_of(r) + " has no usable capacityBytes / freeSpaceBytes")
        cap += c
        free += f
        n += 1
    if n == 0:
        return respond(None, "No JET-based (local) repository was found")
    pct = round((cap - free) * 100.0 / cap, 2)
    return respond(pct, str(pct) + "% used across " + str(n) + " local repositories", {"localRepositories": n})
