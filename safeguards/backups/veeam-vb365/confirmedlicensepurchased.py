"""
confirmedLicensePurchased - Veeam Backup for Microsoft 365

The installed license is Valid and of a purchased type (Subscription, Rental, AwsPrivateOffer), not Evaluation, NFR or Community (GET /v8/License).
...
def evaluate(input):
    ...
    if status is None or kind is None:
        return respond(False, "License response has no status or type")
    if status != "Valid":
        return respond(False, "License status is " + str(status))
    if kind not in ["Subscription", "Rental", "AwsPrivateOffer"]:
        return respond(False, "License type " + str(kind) + " is not a purchased license")
    return respond(True, "Valid " + str(kind) + " license, expires " + str(body.get("licenseExpires")))
