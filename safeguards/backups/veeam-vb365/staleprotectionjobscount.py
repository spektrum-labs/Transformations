"""
staleProtectionJobsCount - Veeam Backup for Microsoft 365

Enabled backup jobs whose last successful backup is older than their schedule interval plus 24 hours, missing, or that run manually only (GET /v8/Jobs).
...
def evaluate(input):
    ...
    every = {"Minutes5": 1, "Minutes10": 1, "Minutes15": 1, "Minutes30": 1, "Hours1": 1, "Hours2": 2, "Hours4": 4,
             "Hours8": 8, "Hours12": 12}
    days = {"Everyday": 24, "Workdays": 72, "Weekends": 144}
    now = utc_now()
    stale = []
    for j in on:
        p = j.get("schedulePolicy") if isinstance(j.get("schedulePolicy"), dict) else {}
        kind = p.get("type")
        if kind == "Periodically":
            hours = every.get(p.get("periodicallyEvery"))
        elif kind == "Daily":
            hours = days.get(p.get("dailyType"), 168 if p.get("dailyType") else None)
        else:
            hours = None
        if hours is None:
            stale.append(name_of(j) + " (no recurring schedule: " + str(kind) + ")")
            continue
        last = parse_time(j.get("lastBackup"))
        if last is None or now - last > timedelta(hours=hours + 24):
            stale.append(name_of(j) + " (last successful backup " + str(j.get("lastBackup")) + ")")
    return respond(len(stale), str(len(stale)) + " of " + str(len(on)) + " enabled backup jobs are behind their schedule",
                   {"enabledBackupJobs": len(on), "staleJobs": stale[:25]})
