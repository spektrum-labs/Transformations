"""
backupSuccessRatePercentage - Veeam Backup for Microsoft 365

Success / finished backup job sessions in the last 7 days x 100 (GET /v8/JobSessions jobType=Backup).

Method: getJobSessions
...
def evaluate(input):
    items, why = page_list(parse_body(input), "backup job sessions (GET /v8/JobSessions)")
    if why:
        return respond(None, why)
    oldest = utc_now() - timedelta(days=8)
    finished = 0
    ok = 0
    for s in items:
        if s.get("jobType") != "Backup":
            return respond(None, "A " + str(s.get("jobType")) + " session was returned, so the jobType filter was not applied")
        created = parse_time(s.get("creationTime"))
        if created is None:
            return respond(None, "Session " + str(s.get("id")) + " has no readable creationTime")
        if created < oldest:
            return respond(None, "A session older than 7 days was returned, so the date filter was not applied")
        st = s.get("status")
        if st in ["Running", "Queued"]:
            continue
        if st not in ["Success", "Warning", "Failed", "Stopped", "Disconnected"]:
            return respond(None, "Session " + str(s.get("id")) + " has status " + str(st))
        finished += 1
        if st == "Success":
            ok += 1
    if finished == 0:
        return respond(None, "No backup job session finished in the last 7 days, so there is no success rate")
    rate = round(ok * 100.0 / finished, 2)
    return respond(rate, str(ok) + " of " + str(finished) + " backup job sessions finished in the last 7 days ended Success",
                   {"finishedSessions": finished, "successfulSessions": ok})
