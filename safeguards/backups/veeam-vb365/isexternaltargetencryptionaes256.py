"""
isExternalTargetEncryptionAES256 - Veeam Backup for Microsoft 365

Every object storage repository is encrypted; VB365 object storage encryption is 256-bit AES in CBC mode (Veeam user guide, Data Encryption).
...
def evaluate(input):
    ...
    bad = [name_of(r) for r in objs if r.get("objectStorageEncryptionEnabled") is not True]
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(objs)) + " object storage repositories are not encrypted", {"repositories": bad[:25]})
    return respond(True, "All " + str(len(objs)) + " object storage repositories are encrypted (VB365 uses 256-bit AES, CBC mode)")
