"""
isBackupTypesScheduled - Veeam Backup for Microsoft 365

Every enabled backup job has a Daily or Periodically schedule (not ManualOnly), and every enabled backup copy job has a schedule (GET /v8/Jobs, /v8/CopyJobs).

Method: getJobInventory
...
def evaluate(input):
    ...
    jobs, why = page_list(parse_body(body.get("jobs")), "backup jobs (GET /v8/Jobs)")
    if why:
        return respond(False, why)
    copies, why = page_list(parse_body(body.get("copyJobs")), "backup copy jobs (GET /v8/CopyJobs)")
    if why:
        return respond(False, why)
    on, why = enabled(jobs, "backup jobs")
    if why:
        return respond(False, why)
    con, why = enabled(copies, "backup copy jobs")
    if why:
        return respond(False, why)

    if len(on) == 0:
        return respond(False, "No enabled backup job was found")
    bad = [name_of(j) + " (" + str(dig(j, "schedulePolicy.type")) + ")" for j in on if dig(j, "schedulePolicy.type") not in ["Daily", "Periodically"]]
    bad = bad + [name_of(j) + " (copy, " + str(dig(j, "schedulePolicy.type")) + ")" for j in con
                 if dig(j, "schedulePolicy.type") not in ["Immediate", "Periodically", "DailyAtTime"]]
    if bad:
        return respond(False, str(len(bad)) + " enabled jobs have no recurring schedule", {"jobs": bad[:25]})
    return respond(True, "All " + str(len(on)) + " enabled backup jobs and " + str(len(con)) + " enabled backup copy jobs run on a schedule")
