"""
isDataLockComplianceModeEnabled - Veeam Backup for Microsoft 365

Every object storage repository is immutable in Compliance mode (enableImmutabilityGovernanceMode false).
...
def evaluate(input):
    ...
    bad = [name_of(r) for r in objs if not (dig(r, "objectStorage.enableImmutability") is True and
                                           dig(r, "objectStorage.enableImmutabilityGovernanceMode") is False)]
    if bad:
        return respond(False, str(len(bad)) + " of " + str(len(objs)) + " object storage repositories are not immutable in Compliance mode", {"repositories": bad[:25]})
    return respond(True, "All " + str(len(objs)) + " object storage repositories are immutable in Compliance mode")
