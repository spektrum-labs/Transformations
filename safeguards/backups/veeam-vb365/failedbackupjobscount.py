"""
failedBackupJobsCount - Veeam Backup for Microsoft 365

Enabled backup and backup copy jobs whose latest status is Failed (GET /v8/Jobs, /v8/CopyJobs lastStatus).
...
def evaluate(input):
    ...
    alljobs = on + con
    if len(alljobs) == 0:
        return respond(None, "No enabled backup or backup copy job was found, so the failure count is not proven")
    failed = []
    for j in alljobs:
        st = j.get("lastStatus")
        if not isinstance(st, str):
            return respond(None, "Job " + name_of(j) + " has no lastStatus")
        if st == "Failed":
            failed.append(name_of(j))
    return respond(len(failed), str(len(failed)) + " of " + str(len(alljobs)) + " enabled backup and backup copy jobs last ended Failed",
                   {"enabledJobs": len(alljobs), "failedJobs": failed[:25]})
