# isransomwaredetectionenabled.py - NetBackup (Cohesity NetBackup, formerly Veritas NetBackup)
#
...
def transform(input):
    """
    Returns isRansomwareDetectionEnabled = True when backup anomaly detection is enabled for all policies
    (backupAnomalyDetection.currentConfigState 2). 1 (unstructured data only) and 3 (disabled) are False.
    """
    key = "isRansomwareDetectionEnabled"
    SETTING = "backupAnomalyDetection"
    ...
    try:
        ...
        if state == 2 and not isinstance(state, bool):
            return {key: True, "reason": "Backup anomaly detection is enabled for all policies"}
        if state == 1 and not isinstance(state, bool):
            return {key: False, "reason": "Backup anomaly detection covers unstructured-data policies only"}
        return {key: False, "reason": "Backup anomaly detection state is " + str(state)}
