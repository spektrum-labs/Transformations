# backupsuccessratepercentage.py - Cohesity DataProtect (Helios / Cohesity Data Cloud)
#
...
def transform(input):
    """
    Returns backupSuccessRatePercentage = active protection groups whose last backup run Succeeded (or
    SucceededWithWarning) / active groups whose last run finished x 100. Runs still in progress are left out. None
    when no last run finished, the list is incomplete or the body is unreadable.
    """
    ...
    try:
        ...
        for g in live:
            ...
            elif status in ["Failed", "Canceled", "Missed", "Skipped", "Partial"]:
                done = done + 1
        if done == 0:
            return {key: None, "reason": "No active protection group has a finished last backup run"}
        return {key: round(ok * 100.0 / done, 2), "reason": str(ok) + " of " + str(done) + " finished last backup runs succeeded"}
