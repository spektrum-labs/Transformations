...
Evaluates whether every active endpoint is monitored by the Sophos MDR service.
An endpoint counts as covered when the Sophos core agent is installed and Sophos has put it
under MDR: mdrManaged is true, or the MDR products ("mtr" Managed Threat Response, "xdr")
are installed. Sophos assigns those only on a licensed MDR subscription.
...
What this proves: every endpoint reporting to Sophos Central is inside the MDR monitoring
scope. What it does not prove: that devices which never installed the Sophos agent are
covered; the endpoint list cannot see them.

Verdict: true when at least one active endpoint exists and all active endpoints are covered.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isEndpointCoverageValid", "vendor": "Sophos Central - MDR", "category": "MDR"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        covered = [e for e in active if mdr_covered(e)]
        uncovered = [label(e) for e in active if not mdr_covered(e)]
        pct = round((len(covered) / len(active)) * 100) if active else 0
        value = len(active) > 0 and len(uncovered) == 0

        summary = {
            "activeEndpoints": len(active),
            "mdrCoveredEndpoints": len(covered),
            "mdrCoveragePercentage": pct,
            "endpointsOutsideMDR": uncovered[:20],
        ...
        if not active:
            ...
            pass_reasons.append(f"All {len(active)} active endpoint(s) are under Sophos MDR monitoring")
        else:
            fail_reasons.append(f"{len(uncovered)} of {len(active)} active endpoint(s) are not under Sophos MDR monitoring")
            recommendations.append("Assign the Sophos MDR licence to: " + ", ".join(str(h) for h in uncovered[:20]))
