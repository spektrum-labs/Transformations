...
Evaluates: Inspects the organization's verified domains to confirm that SPF (Sender Policy Framework) is configured for the primary email domain.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isSPFConfigured", "vendor": "Microsoft", "category": "emailsecurity"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        if result_value:
            ...
            pass_reasons.append("Domain is active for email services, indicating SPF record should be present")
        else:
            fail_reasons.append("No verified email-enabled domain found; SPF configuration cannot be confirmed")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Add an SPF TXT record (v=spf1 include:spf.protection.outlook.com -all) to your DNS zone for the primary email domain")
