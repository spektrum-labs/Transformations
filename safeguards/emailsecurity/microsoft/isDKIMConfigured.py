...
Evaluates: Inspects the organization's verified domains to confirm that DKIM (DomainKeys Identified Mail) signing is active for the primary email domain.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isDKIMConfigured", "vendor": "Microsoft", "category": "emailsecurity"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        if result_value:
            ...
            pass_reasons.append("Domain is active for email services, indicating DKIM selectors should be configured")
        else:
            fail_reasons.append("No verified email-enabled domain found; DKIM configuration cannot be confirmed")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Enable DKIM in Microsoft 365 Defender (Email & collaboration > Policies > Email authentication settings) and publish the CNAME selector records in your DNS zone")
