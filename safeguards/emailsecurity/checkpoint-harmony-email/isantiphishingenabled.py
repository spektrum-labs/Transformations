...
Method: queryPhishingEvents (POST /app/hec-api/v1.0/event/query (requestData.startDate = 30 days ago))

True when at least one security event matching type in ("phishing", "suspicious_phishing") was raised in the last 30 days. Evidence of the engine working, not a read of the policy toggle; a quiet tenant reads false.
...
def transform(input):
    ...
    try:
        ...
        wanted = ("phishing", "suspicious_phishing")
        field = "type"
        ...
        if matched:
            ...
            return create_response(True, pass_reasons=["Harmony Email & Collaboration raised phishing detections in the last 30 days, so its anti-phishing engine is inspecting mail"], input_summary=summary)
        return create_response(False, fail_reasons=["No phishing or suspicious-phishing detection in the last 30 days: anti-phishing activity is not evidenced"], input_summary=summary)
