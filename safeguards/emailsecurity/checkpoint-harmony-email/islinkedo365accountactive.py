"""
Transformation: isLinkedO365AccountActive
Vendor: Check Point Harmony Email & Collaboration (Infinity Portal API)
Method: queryOffice365Events (POST /app/hec-api/v1.0/event/query (requestData.startDate = 30 days ago))

True when at least one security event matching saas in ("office365_emails",) was raised in the last 30 days. Evidence of the engine working, not a read of the policy toggle; a quiet tenant reads false.
...
def transform(input):
    criteriaKey = "isLinkedO365AccountActive"
    ...
    try:
        ...
        if matched:
            ...
            return create_response(True, pass_reasons=["Harmony Email & Collaboration recorded events on the Microsoft 365 mail connector (saas office365_emails) in the last 30 days"], input_summary=summary)
        return create_response(False, fail_reasons=["No event on the Microsoft 365 mail connector in the last 30 days: an active Office 365 link is not evidenced"], input_summary=summary)
