...
Evaluates: Active ZeroFox subscription by verifying user account is active
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "confirmedLicensePurchased", "vendor": "ZeroFox", "category": "Managed Detection & Response"}
        }
    }
...
def evaluate(data):
    ...
    try:
        ...
        if subscription_status in active_statuses:
            result = bool(is_active)
        elif subscription_status in {"suspended", "expired", "cancelled", "inactive"}:
            result = False
        else:
            # subscription_status not present or unknown — fall back to is_active alone
            result = bool(is_active)
