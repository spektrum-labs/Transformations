...
def transform(input):
    """
    Evaluates isConfigurationValid for Zoom.

    Checks: Whether account settings are accessible with the provided OAuth
            credentials via the Server-to-Server OAuth app.
    ...
    try:
        ...
        if error:
            result = False
        elif isinstance(code, int) and code in (124, 200, 300):
            # 124 = invalid access token, 200/300 = Zoom error codes
            result = False
