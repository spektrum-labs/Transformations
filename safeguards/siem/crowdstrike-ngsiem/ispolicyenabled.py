# ispolicyenabled.py - CrowdStrike Falcon Next-Gen SIEM (Security Operations, 1e1212b3)
#
...
def transform(input):
    """
    True when at least one NG-SIEM correlation rule has status "active". False on none, or any unreadable body.
    ...
    try:
        ...
        if not isinstance(data, dict) or not isinstance(data.get("meta"), dict) or not isinstance(data.get("resources", []), list):
            return {key: False, "reason": "Response is not a CrowdStrike MSA body"}
        if data.get("errors"):
            return {key: False, "reason": "CrowdStrike returned errors", "errors": data.get("errors")}
        rules = data.get("resources") or []
        for r in rules:
            if not isinstance(r, dict) or str(r.get("status") or "").lower() != "active":
                return {key: False, "reason": "A returned rule is not active: the status filter was not applied"}
        ...
        if isinstance(n, bool) or not isinstance(n, int) or n < len(rules):
            return {key: False, "reason": "meta.pagination.total is missing or smaller than the rules returned"}
        return {key: n >= 1, "activeRules": n}
