...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isAntiPhishingEnabled", "vendor": "Microsoft", "category": "Email Security"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        if is_enabled:
            policy_names = [p.get("Name", p.get("Identity", "unnamed")) for p in list(enabled_policies[i] for i in range(min(5, len(enabled_policies))))]
