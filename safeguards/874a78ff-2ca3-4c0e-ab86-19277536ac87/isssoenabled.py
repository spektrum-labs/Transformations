...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isSSOEnabled", "vendor": "Microsoft", "category": "Identity"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        if is_enabled:
            provider_names = [p.get('displayName', p.get('name', 'unnamed')) for p in list(providers[i] for i in range(min(5, len(providers))))]
