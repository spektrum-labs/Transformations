...
def transform(input):
    ...
    try:
        ...
        if is_enabled:
            ...
            pass_reasons.append(f"Information Rights Management enabled with {len(active_labels)} active sensitivity label(s): {', '.join(label_names)}")
        else:
            fail_reasons.append("No active sensitivity labels found")
