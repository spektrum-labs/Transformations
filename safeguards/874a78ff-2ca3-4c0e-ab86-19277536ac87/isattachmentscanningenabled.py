...
Evaluates if attachment scanning (Safe Attachments) is enabled based on Microsoft Secure Score.
"""
    ...
    try:
        ...
        if len(values) > 0:
            ...
            if len(matched) == 1:
                ...
                if is_enabled:
                    pass_reasons.append("Attachment scanning is fully enabled (score: 100%)")
                else:
                    fail_reasons.append(f"Attachment scanning score is {score_in_percentage}%")
                    recommendations.append("Enable Safe Attachments policy in Microsoft Defender")
            else:
