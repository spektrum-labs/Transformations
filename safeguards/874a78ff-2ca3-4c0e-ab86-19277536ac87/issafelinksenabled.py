...
Evaluates if Safe Links is enabled based on Microsoft Secure Score.
"""
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isSafeLinksEnabled", "vendor": "Microsoft", "category": "Email Security"}
        }
    }
    ...
    try:
        ...
        if len(values) > 0:
            ...
            if len(matched_object_list) > 1:
                ...
                if is_enabled:
                    pass_reasons.append(f"Safe Links for email is fully enabled (score: 100%)")
                else:
                    fail_reasons.append(f"Safe Links for email score is {score_in_percentage}%")
                    recommendations.append("Enable Safe Links policy in Microsoft Defender for Office 365")
            else:
                fail_reasons.append(f"No control found matching '{controlName}' in Secure Score data")
                recommendations.append("Verify Microsoft Secure Score is collecting Safe Links data")
        else:
