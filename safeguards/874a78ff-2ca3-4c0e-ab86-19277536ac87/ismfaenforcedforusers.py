...
def extract_input(input_data):
    ...
    if isinstance(data, dict):
        ...
        for attempt in range(3):  # Max 3 levels of unwrapping
            unwrapped = False
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isMFAEnforcedForUsers", "vendor": "Microsoft", "category": "Identity"}
        }
    }
...
def transform(input):
    ...
    try:
        ...
        # ----------------------------------------------------------------
            ...
            if is_enabled:
                method_names = [m.get('id', 'unknown') for m in list(enabled_methods[i] for i in range(min(5, len(enabled_methods))))]
