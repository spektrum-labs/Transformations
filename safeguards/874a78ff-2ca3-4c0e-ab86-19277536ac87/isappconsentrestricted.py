...
Evaluates if app consent is restricted based on Microsoft Secure Score.
"""
    ...
    try:
        ...
        if len(values) > 0:
            ...
            if len(matched) == 1:
                ...
                is_restricted = score_in_percentage == 100.00
                count = matched_obj.get("count", 0)
                total = matched_obj.get("total", 0)

                if is_restricted:
                    pass_reasons.append("App consent is properly restricted (score: 100%)")
                else:
                    fail_reasons.append(f"App consent restriction score is {score_in_percentage}%")
                    recommendations.append("Configure app consent policies to restrict user consent to apps")
            else:
        ...
        return create_response(
            result={criteriaKey: is_restricted, "scoreInPercentage": score_in_percentage, "count": count, "total": total},
            validation=validation,
