"""
Transformation: isTransportRuleBannerEnforced
Vendor: Microsoft
Category: Email Security / Transport Rules

Evaluates if transport rule banner is enforced by checking for enabled rules in Enforce mode.
"""

import json
from datetime import datetime


def extract_input(input_data):
    if isinstance(input_data, dict) and "data" in input_data and "validation" in input_data:
        return input_data["data"], input_data["validation"]
    data = input_data
    if isinstance(data, dict):
        wrapper_keys = ["api_response", "response", "result", "apiResponse", "Output"]
        for _ in range(3):
            unwrapped = False
            for key in wrapper_keys:
                if key in data and isinstance(data.get(key), dict):
                    data = data[key]
                    unwrapped = True
                    break
            if not unwrapped:
                break
    return data, {"status": "unknown", "errors": [], "warnings": ["Legacy input format"]}


def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
                    recommendations=None, input_summary=None, transformation_errors=None, api_errors=None):
    if validation is None:
        validation = {"status": "unknown", "errors": [], "warnings": []}
    return {
        "transformedResponse": result,
        "additionalInfo": {
            "validationStatus": validation.get("status", "unknown"),
            "validationErrors": validation.get("errors", []),
            "validationWarnings": validation.get("warnings", []),
            "transformationErrors": transformation_errors or [],

            "apiErrors": api_errors or [],
            "passReasons": pass_reasons or [],

            "failReasons": fail_reasons or [],
            "recommendations": recommendations or [],
            "inputSummary": input_summary or {},
            "metadata": {
                "evaluatedAt": datetime.utcnow().isoformat() + "Z",
                "schemaVersion": "1.0",
                "transformationId": "isTransportRuleBannerEnforced",
                "vendor": "Microsoft",
                "category": "Email Security"
            }
        }
    }


def transform(input):
    criteriaKey = "isTransportRuleBannerEnforced"

    try:
        if isinstance(input, str):
            input = json.loads(input)
        elif isinstance(input, bytes):
            input = json.loads(input.decode("utf-8"))

        data, validation = extract_input(input)

        if validation.get("status") == "failed":
            return create_response(
                result={criteriaKey: False},
                validation=validation,
                fail_reasons=["Input validation failed"]
            )

        pass_reasons = []
        fail_reasons = []
        recommendations = []

        transport_rules = data.get("transportRules", [])
        banner_rules = []

        for rule in transport_rules:
            if rule.get('State') == 'Enabled':
                actions = rule.get('Actions', [])
                if actions:
                    matching_actions = [action for action in actions if ('Disclaimer' in action or 'SubjectPrefix' in action)]
                    if matching_actions:
                        banner_rules.append({
                            'Name': rule.get('Name', 'Unknown'),
                            'Mode': rule.get('Mode', 'Unknown'),
                            'State': rule.get('State', 'Unknown')
                        })

        # Check if any banner rule is in Enforce mode
        enforced_rules = [rule for rule in banner_rules if rule['Mode'] == 'Enforce']
        is_enforced = len(enforced_rules) > 0

        if is_enforced:
            rule_names = [r['Name'] for r in enforced_rules[:3]]
            pass_reasons.append(f"Banner mode enforced via {len(enforced_rules)} rule(s): {', '.join(rule_names)}")
        else:
            if len(banner_rules) > 0:
                fail_reasons.append(f"Found {len(banner_rules)} enabled banner rule(s) but none are in Enforce mode")
            else:
                fail_reasons.append("No enabled transport rules with Disclaimer or SubjectPrefix actions found")
            recommendations.append("Set transport rules with banner/disclaimer actions to Enforce mode")

        return create_response(
            result={criteriaKey: is_enforced, "bannerRulesCount": len(banner_rules), "enforcedRulesCount": len(enforced_rules)},
            validation=validation,
            pass_reasons=pass_reasons,
            fail_reasons=fail_reasons,
            recommendations=recommendations,
            input_summary={"totalRules": len(transport_rules), "enabledBannerRules": len(banner_rules), "enforcedRules": len(enforced_rules)}
        )

    except Exception as e:
        return create_response(
            result={criteriaKey: False},
            validation={"status": "error", "errors": [], "warnings": []},
            transformation_errors=[str(e)],
            fail_reasons=[f"Transformation error: {str(e)}"]
        )
