...
def transform(input):
    ...
    try:
        ...
        if is_enforced:
            ...
            pass_reasons.append(f"Banner mode enforced via {len(enforced_rules)} rules: {', '.join(rule_names)}")
        else:
            if len(banner_rules) > 0:
                fail_reasons.append(f"Found {len(banner_rules)} enabled banner rules but none are in Enforce mode")
            else:
