...
Evaluates if auto-forwarding is disabled based on Microsoft Secure Score.
"""
    ...
    try:
        ...
        if len(values) > 0:
            ...
            if len(matched) == 1:
                ...
                if is_disabled:
                    pass_reasons.append("Auto-forwarding is disabled (score: 100%)")
                else:
                    fail_reasons.append(f"Auto-forwarding restriction score is {score_in_percentage}%")
                    recommendations.append("Disable automatic email forwarding to external domains")
            else:
