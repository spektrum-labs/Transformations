...
Evaluates: Whether Office macro execution is blocked by default.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    response_metadata = {
        ...
        "schemaVersion": "1.0",
        "transformationId": "isMacroBlockingEnabled",
...
def evaluate(data):
    ...
    try:
        file_types_blocked = data.get('EnableFileFilter', False)
        blocked_types = data.get('FileTypes', [])
        macro_blocked = any(
            ext in blocked_types for ext in ['.docm', '.xlsm', '.pptm', '.dotm']
        ) or file_types_blocked
        return {"isMacroBlockingEnabled": macro_blocked, "fileFilterEnabled": file_types_blocked}
...
def transform(input):
    """
    Checks if Office macro execution is blocked by default.
