    ...
    try:
        ...
        if is_allowed:
            if len(enabled_methods) > 0:
                method_names = [m['id'] for m in list(enabled_methods[i] for i in range(min(5, len(enabled_methods))))]
            ...
            insecure_names = [m['id'] for m in list(insecure_methods[i] for i in range(min(5, len(insecure_methods))))]
