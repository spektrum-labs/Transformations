...
Evaluates: Whether hosted content filter (spam) policies are enabled.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    response_metadata = {
        ...
        "schemaVersion": "1.0",
        "transformationId": "isEmailFilteringEnabled",
...
def evaluate(data):
    ...
    try:
        ...
        enabled = any(p.get('IsEnabled', False) for p in policies)
        return {"isEmailFilteringEnabled": enabled, "policyCount": len(policies)}
...
def transform(input):
    """
    Checks if hosted content filter (spam) policy is enabled.
