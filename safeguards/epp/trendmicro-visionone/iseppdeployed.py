...
features are on, or anything about devices Vision One has never seen.
"""
import json
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if value:
            pass_reasons.append("All %d endpoints have the Trend Micro protection agent installed" % len(endpoints))
        else:
            fail_reasons.append("%d of %d endpoints have no Trend Micro protection agent: %s" % (len(missing), len(endpoints), ", ".join(missing[:10])))
            recommendations.append("Deploy the Trend Vision One Endpoint Security agent to the listed endpoints, or remove retired ones from Endpoint Inventory")
