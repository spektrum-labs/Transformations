...
criticality as the customer defines it.
"""
import json
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if not servers:
            fail_reasons.append("No endpoint of type server is in the inventory, so protection of critical systems is not shown")
            recommendations.append("If servers exist, deploy Server & Workload Protection so they appear in Endpoint Inventory")
        elif unprotected:
