"""
Transformation: endpointOperationalStatusUnprotectedCount
...
Verdict: the number of endpoints with no installed protection agent (no eppAgent block, or a
placeholder block with neither version nor protection manager) or whose agent status is off.
None (not a count) when any installed agent reports a status other than on or off, because
those endpoints cannot be placed on either side.

What this proves: how many inventoried endpoints are unprotected. What it does not prove:
anything about devices Vision One has never seen.
...
def transform(input):
    criteriaKey = "endpointOperationalStatusUnprotectedCount"
    try:
        ...
        for e in endpoints:
            if not has_protection_agent(e):
                unprotected.append(endpoint_name(e))
                continue
            status = sub(e, "eppAgent").get("status")
            if status == "off":
                unprotected.append(endpoint_name(e))
            elif status != "on":
                unknown.append(endpoint_name(e))
        value = len(unprotected) if len(unknown) == 0 else None
        summary = {"totalEndpoints": len(endpoints), "knownUnprotected": len(unprotected),
                   "agentStatusUnknown": len(unknown), "sampleUnprotected": unprotected[:10],
                   "sampleUnknown": unknown[:10]}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if unknown:
            fail_reasons.append("%d protection agents report no known status, so the unprotected count is not known (at least %d unprotected): %s" % (len(unknown), len(unprotected), ", ".join(unknown[:10])))
            recommendations.append("Make the protection manager report agent status to Vision One")
        elif unprotected:
            fail_reasons.append("%d of %d endpoints are unprotected: %s" % (len(unprotected), len(endpoints), ", ".join(unprotected[:10])))
            recommendations.append("Install or reconnect the protection agent on the listed endpoints")
        else:
            pass_reasons.append("All %d endpoints have a protection agent with status on" % len(endpoints))
