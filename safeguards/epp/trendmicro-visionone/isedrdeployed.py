...
Verdict: true when every endpoint in the inventory has an endpoint sensor (edrSensor
block) whose status is "enabled". "enabling", "disabling", "disabled", "unknown" and a
missing sensor all fail it.

What this proves: Endpoint Sensor Detection and Response is on for every endpoint Vision
One knows about. What it does not prove: that the sensor is currently connected (see
offlineSensorCount) or anything about devices Vision One has never seen.
"""
import json
...
def transform(input):
    ...
    try:
        ...
        for e in endpoints:
            ...
            status = str(sensor.get("status")) if sensor else "no sensor"
            if status != "enabled":
                not_enabled.append(endpoint_name(e) + " (" + status + ")")
        value = len(not_enabled) == 0
        summary = {"totalEndpoints": len(endpoints), "endpointsWithoutEnabledSensor": len(not_enabled),
                   "sampleWithoutEnabledSensor": not_enabled[:10]}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if value:
            pass_reasons.append("Endpoint Sensor Detection and Response is enabled on all %d endpoints" % len(endpoints))
        else:
            fail_reasons.append("%d of %d endpoints do not have an enabled endpoint sensor: %s" % (len(not_enabled), len(endpoints), ", ".join(not_enabled[:10])))
            recommendations.append("Enable Endpoint Sensor Detection and Response on the listed endpoints in Trend Vision One Endpoint Inventory")
