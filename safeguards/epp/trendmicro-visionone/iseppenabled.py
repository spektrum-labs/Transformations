...
Verdict: true when every endpoint has an installed protection agent (an eppAgent block with a
version or protection manager) whose status is on. eppAgent.status is the protection agent's
connectivity status (on, off, unknown); off, unknown, a missing agent or a placeholder agent
block all fail it.

What this proves: every inventoried endpoint's protection agent is running and reporting to
Vision One. What it does not prove: which protection features are switched on inside it.
    ...
    try:
        ...
        for e in endpoints:
            ...
            if status == "on":
                ...
                off.append(endpoint_name(e))
            else:
                unknown.append(endpoint_name(e))
        value = on == len(endpoints)
        summary = {"totalEndpoints": len(endpoints), "agentsOn": on, "agentsOff": len(off),
                   "agentsStatusUnknown": len(unknown), "endpointsWithoutProtectionAgent": len(no_agent),
                   "sampleOff": off[:10], "sampleUnknown": unknown[:10], "sampleWithoutProtectionAgent": no_agent[:10]}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if value:
            ...
            if off:
                fail_reasons.append("%d protection agents report status off: %s" % (len(off), ", ".join(off[:10])))
            if unknown:
                fail_reasons.append("%d protection agents report no known status, so they are not shown to be running: %s" % (len(unknown), ", ".join(unknown[:10])))
            recommendations.append("Reconnect or reinstall the protection agent on the listed endpoints; agents managed by a connected product must report status to Vision One")
