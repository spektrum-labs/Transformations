"""
Transformation: isolatedEndpointCount
...
Value: the number of endpoints with isolationStatus "on". If any endpoint reports
isolationStatus "unknown" or none at all, the count is not known and the value is None.

What this proves: how many endpoints Vision One currently has network-isolated.
What it does not prove: why they were isolated.
"""
import json
...
def transform(input):
    criteriaKey = "isolatedEndpointCount"
    try:
        ...
        unknown = []
        for e in endpoints:
            status = e.get("isolationStatus")
            if status == "on":
                isolated.append(endpoint_name(e))
            elif status != "off":
                unknown.append(endpoint_name(e))
        value = len(isolated) if len(unknown) == 0 else None
        summary = {"totalEndpoints": len(endpoints), "isolatedEndpoints": len(isolated),
                   "isolationUnknown": len(unknown), "sampleIsolated": isolated[:10], "sampleUnknown": unknown[:10]}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if unknown:
            fail_reasons.append("%d endpoints report no known isolation status, so the isolated count is not known: %s" % (len(unknown), ", ".join(unknown[:10])))
        elif isolated:
            fail_reasons.append("%d endpoints are network-isolated: %s" % (len(isolated), ", ".join(isolated[:10])))
