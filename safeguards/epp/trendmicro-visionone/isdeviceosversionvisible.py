"""
Transformation: isDeviceOSVersionVisible
...
What this proves: the inventory shows the OS name and version of every device it lists.
What it does not prove: that the OS is supported or patched.
...
def transform(input):
    criteriaKey = "isDeviceOSVersionVisible"
    try:
        ...
        for e in endpoints:
            name = e.get("osName")
            version = e.get("osVersion")
            if not (isinstance(name, str) and name.strip()) or not (isinstance(version, str) and version.strip()):
                missing.append(endpoint_name(e))
        value = len(missing) == 0
        summary = {"totalEndpoints": len(endpoints), "endpointsWithoutOsVersion": len(missing),
                   "sampleWithoutOsVersion": missing[:10]}
        pass_reasons = []
        fail_reasons = []
        recommendations = []
        if value:
            pass_reasons.append("All %d endpoints report an OS name and version" % len(endpoints))
        else:
            fail_reasons.append("%d of %d endpoints report no OS name or version: %s" % (len(missing), len(endpoints), ", ".join(missing[:10])))
            recommendations.append("Check agent connectivity on the listed endpoints so Endpoint Inventory can read their OS")
