...
Evaluates: Checks tenant domain configuration records for SPF TXT entries to confirm SPF is properly set up.
"""
import json
...
def create_response(result, validation=None, pass_reasons=None, fail_reasons=None,
    ...
    return {
        ...
        "additionalInfo": {
            ...
            "metadata": {"evaluatedAt": datetime.utcnow().isoformat() + "Z", "schemaVersion": "1.0", "transformationId": "isSPFConfigured", "vendor": "Microsoft", "category": "claims-defense"}
        }
    }
    ...
    try:
        ...
        for domain in domains:
            ...
            if domain_has_spf(domain):
                spf_configured_domains.append(domain_id)
            else:
                is_verified = domain.get("isVerified", False)
                if is_verified:
                    non_configured_verified_domains.append(domain_id)
        spf_configured = len(spf_configured_domains) > 0
        return {
            "isSPFConfigured": spf_configured,
            "totalDomains": total_domains,
            "spfConfiguredDomains": spf_configured_domains,
...
def transform(input):
    ...
    try:
        ...
        recommendations = []
        if result_value:
            pass_reasons.append("SPF TXT records are configured for at least one domain.")
            pass_reasons.append("SPF-configured domains: " + str(extra_fields.get("spfConfiguredDomains", [])))
        else:
            fail_reasons.append("No SPF TXT records detected in tenant domain configuration.")
            if "error" in eval_result:
                fail_reasons.append(eval_result["error"])
            recommendations.append("Add a valid 'v=spf1' TXT record for each verified domain to prevent email spoofing.")
