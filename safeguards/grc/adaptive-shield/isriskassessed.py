...
def transform(input):
    """
    Evaluates isRiskAssessed for Adaptive Shield

    Checks: Whether posture risks are identified and assessed
    API Source: {baseURL}/api/v1/posture/risks
    Pass Condition: At least 1 risk entry exists with a severity or score assigned
    ...
    try:
        ...
        assessed = [
            r for r in risks
            if r.get("severity") or r.get("score") or r.get("risk_score") or r.get("riskLevel")
        ]
