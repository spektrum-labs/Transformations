...
Evaluates: count of assets ranked high; true when 0
...
def matches(a):
    return str(a.get("bd.severity_ranking") or "").lower() == "high"
...
def evaluate(data):
    ...
    if not assets:
        # See nocriticalfindings.py: `read_assets` returns [] for an unreadable body, so
...
def transform(input):
    return run_criterion(input, "noHighFindings", evaluate, "noHighFindings")
