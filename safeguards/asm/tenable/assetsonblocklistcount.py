"""
Transformation: assetsOnBlocklistCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: assets appearing on a realtime blocklist -- reputation damage or compromise
Reads: rbls.rbls
...
def matches(a):
    return bool(as_list(a, "rbls.rbls"))
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) listed on a realtime blocklist across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) listed on a realtime blocklist"], ['Investigate: a blocklisted host usually means compromise or abuse originating from it'], []


def transform(input):
    return run_criterion(input, "assetsOnBlocklistCount", evaluate, "assetsOnBlocklistCount")
