...
def _match(a):
    return any(int(p) in ADMIN_PORTS for p in _listy(a, "ports.ports") if str(p).isdigit())
...
def transform(input):
    return _run(input, "exposedAdminPortsCount", evaluate, "exposedAdminPortsCount")
