"""
Transformation: expiredCertificateCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: assets serving an already-expired certificate
Reads: ssl.valid_to
...
def matches(a):
    vt = parse_iso_date(a.get("ssl.valid_to"))
    return bool(vt and vt < datetime.now(timezone.utc))
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) serving an expired certificate across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) serving an expired certificate"], ['Renew or retire the expired certificates'], []


def transform(input):
    return run_criterion(input, "expiredCertificateCount", evaluate, "expiredCertificateCount")
