...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) with an SSL/TLS error across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) with an SSL/TLS error"], ['Fix the certificate chain or hostname mismatch on the affected assets'], []


def transform(input):
    return run_criterion(input, "sslErrorCount", evaluate, "sslErrorCount")
