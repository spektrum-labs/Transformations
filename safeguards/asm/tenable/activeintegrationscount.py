"""
Transformation: activeIntegrationsCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getSources (GET /sources)
Evaluates: number of discovery sources feeding the inventory
Reads: /sources
...
def evaluate(data):
    sources = list_items(data, "sources", "items", "data")
    if sources is None:
        return 0, {"count": 0}, [], ["/sources returned no readable list"], ["Confirm the API key"], ["source list unreadable"]
    kinds = {}
    for s in sources:
        if isinstance(s, dict):
            k = str(s.get("type") or s.get("kind") or s.get("sourceType") or "unknown")
            kinds[k] = kinds.get(k, 0) + 1
    n = len(sources)
    extras = {"count": n, "sourceTypes": kinds}
    if n > 0:
        return n, extras, [f"{n} discovery source(s) configured"], [], [], []
    return 0, extras, [], ["no discovery sources configured"], ["Add at least one domain, ASN or keyword source"], []


def transform(input):
    return run_criterion(input, "activeIntegrationsCount", evaluate, "activeIntegrationsCount")
