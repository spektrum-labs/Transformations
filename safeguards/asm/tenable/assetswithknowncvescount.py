...
def matches(a):
    return bool(as_list(a, "ports.cves"))
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) exposing a service with a known CVE across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) exposing a service with a known CVE"], ['Patch or remove the affected services; these are internet-reachable'], []


def transform(input):
    return run_criterion(input, "assetsWithKnownCvesCount", evaluate, "assetsWithKnownCvesCount")
