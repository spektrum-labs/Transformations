...
Evaluates: inventory total > 0: ASM is enabled and holds discovered assets
Reads: total, stats
...
def evaluate(data):
    ...
    extras = {"assetTotal": total, "hostCount": stats.get("hostcount"), "domainCount": stats.get("domaincount"),
              "subdomainCount": stats.get("subdomaincount"), "ipCount": stats.get("ipcount")}
    if total is None:
        return False, extras, [], ["inventory response carried no total and no assets"], ["Confirm the API key has access to this inventory"], ["unreadable inventory response"]
    if total > 0:
        return True, extras, [f"inventory holds {total} asset(s)"], [], [], []
    return False, extras, [], ["inventory is empty"], ["Add discovery sources (domains, ASNs, keywords) so ASM has something to inventory"], []


def transform(input):
    return run_criterion(input, "isASMEnabled", evaluate, "isASMEnabled")
