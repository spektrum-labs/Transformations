...
def evaluate(data):
    ...
    for a in assets:
        if not isinstance(a, dict):
            continue
        added = _iso(a.get("bd.addedtoportfolio"))
        if not added or (now - added).total_seconds() / 86400 > WINDOW_DAYS:
            continue
        recent += 1
        if not (_listy(a, "bd.tags") or _listy(a, "bd.string_custom_columns") or _listy(a, "bd.custom_columns")):
            untriaged += 1
            if len(sample) < 25:
...
def transform(input):
    return _run(input, "assetTriageBacklogCount", evaluate, "assetTriageBacklogCount")
