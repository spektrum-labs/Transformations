"""
Transformation: assetTriageBacklogCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: assets added in the last 30 days still carrying no tag or custom column -- untriaged discovery
Reads: bd.addedtoportfolio, bd.tags, bd.string_custom_columns, bd.custom_columns
...
def evaluate(data):
    ...
    recent, untriaged, sample = 0, 0, []
    for a in assets:
        if not isinstance(a, dict):
            continue
        added = parse_iso_date(a.get("bd.addedtoportfolio"))
        if not added or (now - added).total_seconds() / 86400 > WINDOW_DAYS:
            continue
        recent += 1
        if not (as_list(a, "bd.tags") or as_list(a, "bd.string_custom_columns") or as_list(a, "bd.custom_columns")):
            untriaged += 1
            if len(sample) < 25:
                sample.append(a.get("bd.original_hostname"))
    extras = {"count": untriaged, "addedInWindow": recent, "assetsScanned": len(assets),
              "inventoryTotal": total, "windowDays": WINDOW_DAYS, "sample": sample}
    if partial:
        extras["partial"] = True
    if recent == 0:
        return 0, extras, [f"no assets added in the last {WINDOW_DAYS} days"], [], [], []
    if untriaged == 0:
        return 0, extras, [f"all {recent} asset(s) added in the last {WINDOW_DAYS} days have been classified"], [], [], []
    return untriaged, extras, [], [f"{untriaged} of {recent} assets added in the last {WINDOW_DAYS} days carry no tag or custom column"], ["Triage newly discovered assets: assign an owner and an environment tag"], []


def transform(input):
    return run_criterion(input, "assetTriageBacklogCount", evaluate, "assetTriageBacklogCount")
