"""
Transformation: isUnmanagedAssetDiscoveryEnabled
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventorySummary (POST /inventory limit=1)
Evaluates: stats.subdomaincount > 0 and total > domaincount: assets discovered beyond the seeded domains
Reads: total, stats.domaincount, stats.subdomaincount
...
def evaluate(data):
    assets, total, stats, partial_unused = read_assets(data)
    dom = int(stats.get("domaincount") or 0)
    sub = int(stats.get("subdomaincount") or 0)
    extras = {"assetTotal": total, "domainCount": dom, "subdomainCount": sub,
              "discoveredBeyondDomains": max((total or 0) - dom, 0)}
    if total is None or not stats:
        return False, extras, [], ["inventory summary carried no stats block"], ["Confirm the API key has access to this inventory"], ["no stats in inventory response"]
    if sub > 0 and total > dom:
        return True, extras, [f"{sub} subdomain(s) and {total - dom} asset(s) beyond the {dom} seeded domain(s)"], [], [], []
    return False, extras, [], ["inventory holds only the seeded domains; nothing unmanaged discovered"], ["Enable subdomain/ASN discovery on the inventory sources"], []


def transform(input):
    return run_criterion(input, "isUnmanagedAssetDiscoveryEnabled", evaluate, "isUnmanagedAssetDiscoveryEnabled")
