...
Evaluates: stats.subdomaincount > 0 and total > domaincount: ASM has discovered assets beyond the seeded domains
...
def evaluate(data):
    assets, total, stats = _assets(data)
    dom = int(stats.get("domaincount") or 0)
    sub = int(stats.get("subdomaincount") or 0)
    extras = {"assetTotal": total, "domainCount": dom, "subdomainCount": sub, "discoveredBeyondDomains": max((total or 0) - dom, 0)}
    ...
    if sub > 0 and total > dom:
        return True, extras, [f"{sub} subdomain(s) and {total - dom} asset(s) beyond the {dom} seeded domain(s) were discovered"], [], [], []
    return False, extras, [], ["inventory holds only the seeded domains; nothing unmanaged has been discovered"], ["Enable subdomain/ASN discovery on the inventory sources"], []
