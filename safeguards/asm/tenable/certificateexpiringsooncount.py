...
Evaluates: certificates expiring within 30 days (already-expired counted separately)
Reads: ssl.valid_to
...
def evaluate(data):
    ...
    extras = {"count": soon, "expiredCount": expired, "assetsWithCertificate": with_cert,
    ...
    if with_cert == 0:
        return 0, extras, ["no certificates observed on the scanned assets"], [], [], []
...
def transform(input):
    return run_criterion(input, "certificateExpiringSoonCount", evaluate, "certificateExpiringSoonCount")
