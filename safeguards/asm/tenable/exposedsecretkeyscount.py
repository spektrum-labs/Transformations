"""
Transformation: exposedSecretKeysCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: assets whose page source leaks API or secret keys
Reads: wtech.secretkeys
...
def matches(a):
    return bool(as_list(a, "wtech.secretkeys"))
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) leaking a secret or API key in page source across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) leaking a secret or API key in page source"], ['Rotate the exposed keys and remove them from client-side source'], []


def transform(input):
    return run_criterion(input, "exposedSecretKeysCount", evaluate, "exposedSecretKeysCount")
