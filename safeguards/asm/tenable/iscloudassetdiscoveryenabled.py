...
Evaluates: assets carrying ipgeo.cloudhosted: the cloud estate is being discovered
Reads: ipgeo.cloudhosted, ipgeo.cloud
...
def evaluate(data):
    ...
    hosted = [a for a in assets if isinstance(a, dict) and a.get("ipgeo.cloudhosted") is True]
    providers = sorted({str(a.get("ipgeo.cloud")) for a in hosted if a.get("ipgeo.cloud")})
    extras = {"cloudHostedCount": len(hosted), "assetsScanned": len(assets), "providers": providers, "inventoryTotal": total}
    if partial:
        extras["partial"] = True
    if hosted:
        return True, extras, [f"{len(hosted)} cloud-hosted asset(s) discovered across {len(providers) or 1} provider(s)"], [], [], []
    return False, extras, [], ["no cloud-hosted assets in the inventory"], ["Connect cloud sources (Azure keys) or confirm the estate has no cloud-hosted exposure"], []


def transform(input):
    return run_criterion(input, "isCloudAssetDiscoveryEnabled", evaluate, "isCloudAssetDiscoveryEnabled")
