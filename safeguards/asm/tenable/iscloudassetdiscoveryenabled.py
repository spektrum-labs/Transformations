"""
Transformation: isCloudAssetDiscoveryEnabled
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getCloudHostedAssets (POST /inventory filter ipgeo.cloudhosted is true, limit=1)
Evaluates: at least one inventory asset is cloud-hosted: cloud discovery is producing assets
...
def evaluate(data):
    ...
    if total is None:
        return False, {"cloudHostedCount": None}, [], ["inventory response unreadable"], ["Confirm the API key"], ["unreadable inventory response"]
    providers = sorted({str(a.get("ipgeo.cloud")) for a in assets if isinstance(a, dict) and a.get("ipgeo.cloud")})
    extras = {"cloudHostedCount": total, "sampleProviders": providers}
    if total > 0:
        return True, extras, [f"{total} cloud-hosted asset(s) discovered"], [], [], []
    return False, extras, [], ["no cloud-hosted assets in the inventory"], ["Connect cloud sources (e.g. Azure keys) or confirm the estate has no cloud-hosted exposure"], []


def transform(input):
    return _run(input, "isCloudAssetDiscoveryEnabled", evaluate, "isCloudAssetDiscoveryEnabled")
