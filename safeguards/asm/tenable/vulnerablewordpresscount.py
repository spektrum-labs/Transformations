"""
Transformation: vulnerableWordPressCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: WordPress assets with WPScan-identified vulnerabilities
Reads: wpscan.vulnerabilities
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no WordPress asset(s) with a known vulnerability across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} WordPress asset(s) with a known vulnerability"], ['Update WordPress core and the flagged plugins'], []
