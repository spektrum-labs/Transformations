"""
Transformation: mixedContentAssetsCount
Vendor: Tenable  |  Category: Attack Surface Management  |  Method: getInventory (POST /inventory, paged)
Evaluates: HTTPS assets loading insecure sub-resources
Reads: wtech.mixedcontent
...
def matches(a):
    return bool(as_list(a, "wtech.mixedcontent"))
...
def evaluate(data):
    ...
    if n == 0:
        return 0, extras, [f"no asset(s) serving mixed content over HTTPS across {len(assets)} scanned asset(s)"], [], [], []
    return n, extras, [], [f"{n} asset(s) serving mixed content over HTTPS"], ['Serve every sub-resource over HTTPS'], []


def transform(input):
    return run_criterion(input, "mixedContentAssetsCount", evaluate, "mixedContentAssetsCount")
