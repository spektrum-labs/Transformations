...
def transform(input):
    """
    Evaluates isSecurityPolicyEnforced for TeamViewer

    Checks: Whether TeamViewer policies are configured and active
    API Source: {baseURL}/teamviewerpolicies
    Pass Condition: At least one policy exists and is assigned
    ...
    try:
        ...
        for p in policies:
            ...
            policyId = p.get("policy_id", p.get("id", ""))
            if enabled is True:
                active.append(p)
