...
def transform(input):
    """
    Evaluates isSecurityPolicyEnforced for Uptycs

    Checks: Whether Uptycs alert rules are configured and enabled
    API Source: {baseURL}/alertRules
    Pass Condition: At least one alert rule exists and is enabled
    ...
    try:
        ...
        # -- EVALUATION LOGIC --
        rules = data.get("items", data.get("data", data.get("alertRules", data.get("results", []))))
        ...
        if not isinstance(rules, list):
            return {
                ...
                "activePolicies": 0,
                "totalPolicies": 0,
                "error": "Unexpected alert rules response format"
            }
