...
def transform(input):
    try:
        ...
        # empty findings list and reported "No unencrypted storage
        # resources" about an estate that was never queried.
        if findings is None:
            return build_response({CRITERIA_KEY: False, "unencryptedResources": -1}, TRANSFORM_ID,
                                  errors=["no Findings collection in the Security Hub GetFindings response: the findings "
                                          "query cannot be shown to have run, so zero unencrypted storage resources "
                                          "is not evidence of an encrypted estate"])
        unencrypted = 0
        for f in findings:
            if not isinstance(f, dict):
                continue
