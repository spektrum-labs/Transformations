...
def transform(input):
    """
    Evaluates confirmedLicensePurchased for ManageEngine ServiceDesk Plus (On-Premise)

    Checks: Whether the ServiceDesk Plus instance returns a valid API response
    API Source: {baseURL}/api/v3/requests?list_info={"row_count":1}
    Pass Condition: API returns a successful response confirming active license
