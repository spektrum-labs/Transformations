...
Judged setting: gmail.links_and_external_images
Required fields (each must equal "True"): enableShortenerScanning, enableAggressiveWarningsOnUntrustedLinks

Google Workspace has no URL rewriting. The nearest control is the Gmail "Links and external images"
safety policy: links behind shorteners are scanned and untrusted links get an aggressive warning at
click time. External image scanning is not required (it covers images, not links).
Same evidence and rule as isurlrewriteenabled.py, which answers a different criterion key.

Rule (fail closed): true only when at least one gmail.links_and_external_images policy is returned and EVERY one of them
...
ADVICE = "In Admin console > Apps > Google Workspace > Gmail > Safety > Links and external images, turn on shortener scanning and aggressive warnings on untrusted links"
