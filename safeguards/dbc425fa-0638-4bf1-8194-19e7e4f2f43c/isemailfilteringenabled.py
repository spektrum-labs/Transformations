...
Judged setting: gmail.enhanced_pre_delivery_message_scanning
Required fields (each must equal "True"): enableImprovedSuspiciousContentDetection

Gmail spam and malware filtering is always on and cannot be switched off, so the judged setting is
the one an admin can turn off: enhanced pre-delivery message scanning (extra content scanning of
suspicious messages before they reach the inbox).

Rule (fail closed): true only when at least one gmail.enhanced_pre_delivery_message_scanning policy is returned and EVERY one of them
...
CRITERIA_KEY = "isEmailFilteringEnabled"
SETTING_TYPE = "gmail.enhanced_pre_delivery_message_scanning"
REQUIRED_FIELDS = ['enableImprovedSuspiciousContentDetection']
CONTROL = "Enhanced pre-delivery scanning"
ADVICE = "In Admin console > Apps > Google Workspace > Gmail > Spam, phishing and malware, turn on enhanced pre-delivery message scanning"
