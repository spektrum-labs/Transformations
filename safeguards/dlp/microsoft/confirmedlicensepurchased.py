def transform(input):
    ...
    Parameters:
        input (dict): The JSON data containing Microsoft DLP API response
    ...
    try:
        ...
        elif 'value' in input and isinstance(input['value'], list):
            # Microsoft Graph API returns subscribed SKUs in value array
            skus = [s for s in input['value'] if s.get('capabilityStatus') == 'Enabled']
            license_purchased = len(skus) > 0
            license_details['subscribedSkus'] = skus
