...
def evaluate(data):
    """Check Air Marshal IPS is enabled across all wireless networks.
    ...
    try:
        ...
        if not am_items:
            return {"isIPSEnabled": False, "networksEvaluated": 0, "error": "No wireless networks found"}

        without_ips = []
        for item in am_items:
            if item.get('defaultPolicy') == 'allow':
                without_ips.append(item.get('networkId', 'unknown'))

        return {
            "isIPSEnabled": len(without_ips) == 0,
            "networksEvaluated": len(am_items),
            "networksWithoutIPS": without_ips
        }
