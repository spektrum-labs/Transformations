...
def evaluate(data):
    """Check Air Marshal IDS is enabled across all wireless networks.
    ...
    try:
        ...
        if not am_items:
            return {"isIDSEnabled": False, "networksEvaluated": 0, "error": "No wireless networks found"}

        without_ids = []
        for item in am_items:
            if item.get('defaultPolicy') == 'allow':
                without_ids.append(item.get('networkId', 'unknown'))

        return {
            "isIDSEnabled": len(without_ids) == 0,
            "networksEvaluated": len(am_items),
            "networksWithoutIDS": without_ids
        }
