def transform(input):
    ...
    Parameters:
        input (dict): The JSON data containing Mark Monitor API response
    ...
    try:
        ...
        # Mark Monitor health check response
        if 'accountId' in input or 'account_id' in input:
            license_purchased = True
            license_details['accountId'] = input.get('accountId', input.get('account_id', ''))
