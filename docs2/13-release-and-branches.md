...
- **There is no gate.** No `.github/`, no CI, no CODEOWNERS on either branch; GitHub branch protection on `main` is **disabled** (re-queried live 2026-09-16); sampled PRs to both branches show 0 reviews and author == merger (the first sampled exceptions, Tenable ASM PRs #559/#560 on 2026-09-16, carried 1 review each — and still shipped 26 sandbox-fatal files); direct pushes to `main` have happened.
...
- **`develop` is +564 commits** — overwhelmingly an automated vendor-onboarding pipeline (30 `feat/transformations-<vendor>-<timestamp>` merges plus 12 auto-generated revert merges, and now three `make-live:` direct pushes — `8bf278fb`, `0783db68`, `d3bece29`; 517 of 564 commits under a single author identity). None of it is reachable by minted production URLs (the branch-pinning caveat is in [Gotchas](#gotchas)).
- **`main` is +93 commits** — almost entirely direct production hotfixes (the remainder is the tail of the April `staging` train), including a 2026-09-01→04 burst (Azure backups PR #531, BeyondTrust PRA PR #533, NinjaOne disk encryption PR #544, Lookout fleet-count PR #548) and a 2026-09-10→16 burst (Okta `auth_types_allowed` PR #551, GitHub DevSecOps PR #556, and Tenable ASM PRs #558–#560 — 26 new transforms, every one sandbox-fatal on arrival, see [14-known-issues.md](14-known-issues.md)).
- **Develop is NOT a superset of main**: 100 files on main carry content develop lacks (72 fixes plus the 27-file Tenable ASM set and the Okta ENG-560 change), and nobody back-merges (zero main→develop merges since the base).
- **A promotion today = 313 touched paths, 305 of them instantly live** (+26,525/−499 outside the conflicts), with 8 real merge conflicts (2 NinjaOne + 6 Lookout), 9 path retirements that 404 stored URLs, and 3 added transforms that are sandbox-fatal on arrival.
...
flowchart LR
    ...
    dev["origin/develop @ d3bece29<br/>+564 commits since base<br/>automated vendor pipeline"]
    prod["origin/main @ 6c0e6e63<br/>+93 commits since base<br/>direct production hotfixes"]
    ...
    prod -. "never back-merged<br/>100 files develop lacks" .-> dev
```
...
- **No branch protection**: `gh api repos/spektrum-labs/Transformations/branches/main` returned `{"protected":false,"protection":{"enabled":false,...}}` on 2026-09-04 and again on 2026-09-16. The only automated check on main's tip is GitHub default-setup CodeQL — not repo-defined, not required, not a test.
- **No review in practice**: sampled PRs #544/#533/#529/#551/#556/#558 (base main) and #530/#528/#511/#550/#555 (base develop) all show 0 reviews, author == merger. The only reviewed PRs found are Tenable ASM #559 and #560 (1 review each, author ≠ merger, 2026-09-16) — review without a sandbox check, so all 26 of their transforms still deploy broken ([14-known-issues.md](14-known-issues.md)).
...
**Raw counts** (measured 2026-09-16 on the two tips):

| Measure | `main..develop` (develop-only) | `develop..main` (main-only) |
|---|---|---|
| Commits | 564 (60 merges, 504 non-merges) | 93 (59 merges, 34 non-merges) |
| Commit dates | 2026-04-22 → 2026-09-15 | 2026-04-14 → 2026-09-16 |
| Files changed since base | 375 (+32,781/−1,680) | 161 (+19,898/−1,523) |
| Character | 428 `add:` commits; 30 timestamped `feat/transformations-*` vendor merges + 12 auto-generated revert merges + 3 `make-live:` direct pushes; 517 of 564 commits by one author identity | 34 direct fixes; 2 direct pushes; after the 04-22 promotion train (14 `staging` merges), first-parent merges are all `hotfix/`, `fix/`, `hf/`, `ENG-*`, `LABS-*`, `feat/tenable-asm-*` |
...
- **main is a hotfix-only production trunk.** Recent examples, all straight to main: Qualys zero-vuln pass fix (`93a929fd`, 2026-07-07), Britive IAM fixes ×3 (2026-07-14), Anthropic RestrictedPython hardening (`a51b8a1f`, ENG-463, 2026-08-25), Azure `isBackupTested` (PR #531, 2026-09-01), BeyondTrust PRA sandbox fix (PR #533, 2026-09-02), NinjaOne disk-encryption rewrite (PR #544, 2026-09-03), Lookout fleet-count fix (PR #548, 2026-09-04, correcting transforms develop had staged reading the page length as the fleet size), Okta `auth_types_allowed` fail-closed fix (ENG-560, PR #551, 2026-09-10), and the Tenable ASM vendor (PRs #558–#560, 2026-09-16 — main's current HEAD: 26 transforms that never existed on develop, every one of which fails RestrictedPython compilation on underscore-prefixed helpers).
...
| 2026-09-09 | develop | Horizon3 NodeZero count-verdict fix (#550) | `0e010c36` |
| 2026-09-10 | main only | ENG-560: Okta `86ded564-…/auth_types_allowed.py` fail-closed rewrite (#551) — develop's copy still equals the merge-base | `354d3f90` |
| 2026-09-14 | both | ENG-576 GitHub DevSecOps twin PRs (#555 develop, #556 main) — `devsecops/github/` lands on main byte-identical to develop; the twin-PR discipline held | `7bf85804` ≡ `8dba7745` |
| 2026-09-15 | develop | Rapid7 vendor `make-live:` direct push — `threat-vulnerability-management/rapid7/`, develop-only | `d3bece29` |
| 2026-09-16 | main only | Tenable ASM PRs #558–#560 — 26 transforms + README, all sandbox-fatal (underscore helpers); the first reviewed PRs in the sample | `03d5ebde`, `ece3b6ac`, `6c0e6e63` |
...
4. *Sometimes* a twin PR lands the same content on develop as a separate patch (the `…-main` / `…-develop` branch-suffix convention). This hand discipline is what keeps 61 of the 70 both-sides-touched files byte-identical (most recently the 11 `devsecops/github/` files, ENG-576, PRs #555/#556).
...
> [!IMPORTANT]
> **"Merged" usually means develop, and develop is not production.** Thirteen vendor directories exist only on develop today (okta, crowdstrike-falcon, sentinelone, red-canary, sonicwall, rapid7, horizon3-nodezero, and six more — [04-catalog.md](04-catalog.md); github left this list on 2026-09-14 when ENG-576 twin-landed it on main) and are invisible to every minted default URL. The one caveat: 16 Integration-Service reference configs pin `refs/heads/develop`, four of them naming develop-only dirs — wherever live DB rows match those copies, a develop push is already production ([04-catalog.md](04-catalog.md)). The reverse also holds: seven vendor dirs (anthropic, redcanary, proofpoint-threat-protection, sophos ×3, and since 2026-09-16 `asm/tenable`) exist only on main, so develop-based work cannot even see them.
...
A promotion was simulated on 2026-09-03 (`git merge --no-ff` of the then-tips), re-simulated on 2026-09-04 (`git merge-tree` of `8bf278fb` into `9d0262aa`), and again on 2026-09-16 (`git merge-tree` of `d3bece29` into `6c0e6e63`). Everything in this section is from the 2026-09-16 simulation, not extrapolation.

```mermaid
flowchart TD
    merge["git merge develop into main<br/>(re-simulated 2026-09-16)"]
    conflicts["8 add/add conflicts<br/>2 ninjaone-endpoint-management (PR #544)<br/>+ 6 mobile-security/lookout (PR #548)"]
    payload["305 files instantly live<br/>+26,525 / −499"]
    adds["276 added: 13 new vendors,<br/>10 pytest files, 3 sandbox-fatal<br/>mfa/azure transforms"]
    ...
    kept["100 main-side files develop lacks:<br/>91 PRESERVED automatically,<br/>9 ride on conflict resolution"]
    merge --> conflicts
    merge --> payload
    payload --> adds
    payload --> mods
...
Walkthrough: the merge itself is mostly mechanical — git auto-resolves 305 of the 313 touched paths correctly — but every one of those files is production the moment the merge commit lands on `refs/heads/main`, with no canary and rollback only by another merge.
...
The scary-sounding half — "promoting develop would revert main's 93 hotfixes" — is **false for a clean merge**, and it's worth being precise about why.

> [!IMPORTANT]
> `git merge` keeps main's hotfixes automatically. For 91 of the 100 main-side files develop lacks, develop never touched them after the base — its copies are byte-identical to the merge-base (or don't exist at all) — so three-way merge takes main's side with no conflict; the other 9 are the qualys collision (auto-resolves to main) and the 8 add/add conflicts (2 NinjaOne + 6 Lookout, hand-resolved). Verified in the simulation: the Qualys, Britive, Azure, BeyondTrust, Anthropic, and Okta ENG-560 fixes all survive (as does the whole broken Tenable ASM set); the qualys collision auto-resolves to main's fixed version because develop's change is a strict subset of main's.
...
<details><summary><b>Deep dive:</b> the full collision analysis (70 both-sides files)</summary>

Files changed on BOTH branches since the merge-base — the honest superset test — number exactly 70 (the 9 `mobile-security/lookout/` files joined the set on 2026-09-04, and the 11 `devsecops/github/` files on 2026-09-14 when ENG-576 twin-landed on both branches):

- **61 are byte-identical at the two tips** (including all 11 github files, and 3 of the 9 lookout files — `deviceManagementState.py` and the 2 schemas): the same fix landed on both branches as separate patches (the twin-PR pattern). 7 pairs are patch-equivalent by `git log --cherry-mark`: Cisco Umbrella, Qualys counts-as-numbers, Sophos epp/mdr, NinjaOne epp feat, NinjaOne patch fix, Netskope, AWS `isBackupTested`.
...
- **Zero back-merges** of main into develop exist anywhere after the 2026-04-22 base; 86 of main's 93 commits have no develop equivalent by patch-id.
...
- **The gap is invisible from inside either branch.** Fix branches carry the target in the name (`…-main`, `…-develop`), so a *missing* twin leaves no trace in either branch's own log — the 100 main-side files develop lacks only show up by diffing tips.
- Eight never-ported changes are known concretely: Qualys zero-pass (`93a929fd`), Britive ×3, Azure restore-history (`e7be0780`), BeyondTrust PRA (`b2e6e623`), NinjaOne-EM (`5ae4693a`), Lookout fleet-count (`382dc385`), Okta `auth_types_allowed` (`354d3f90`, ENG-560), and the entire Tenable ASM vendor (`6c0e6e63`, PRs #558–#560). The one recent counter-example is ENG-576 (GitHub), which did land as twins (#555/#556).
...
1. **Back-merge main into develop first.** Close the 100-file gap so the promotion diff is honest and the twin-PR debt is settled before anything moves toward production. This also converts the 8 add/add conflicts into a develop-side decision made *off* the production branch.
...
3. **Run a sandbox compile scan over the candidate merged tree.** Replicate Token-Service's pipeline — the AST validation plus a real `compile_restricted` — over every non-schema `.py` file. Today that scan flags develop's 3 `mfa/azure` transforms and the 10 pytest files — and, run over main itself, all 26 `asm/tenable/` transforms; it also catches the underscore-helper pattern generally ([12-local-development.md](12-local-development.md) explains why `local_tester.py` cannot do this).
4. **Audit filename and directory casing.** Minted default URLs lowercase both path segments while raw GitHub is case-sensitive; develop adds ~113 new mixed-case filenames (172 vs main's 59). Any new mixed-case file for a minted-default vendor is born unreachable ([02-execution-contract.md](02-execution-contract.md), [04-catalog.md](04-catalog.md)).
5. **Review the collision set explicitly.** Diff the 70 both-sides files at the two tips; confirm the 61 byte-identical, and confirm the qualys file resolves to main's version.
...
> [!WARNING]
> **A missing twin PR is undetectable from either branch alone.** The `…-main`/`…-develop` naming convention means each branch's history looks complete on its own; the 100-file back-port debt accumulated precisely because only a tip-to-tip diff reveals it. If you fix something on main, port it to develop in the same sitting or file the debt somewhere visible.
...
| Merge-base / skew measurements | `git merge-base origin/main origin/develop` → `aa9c82a5`; counts via `git rev-list --count` on fresh `origin/*` refs | 564 vs 93; all numbers in this doc |
...
| Sandbox-fatal main-only vendor | `main:safeguards/asm/tenable/*.py` (26 files, PRs #558–#560, 2026-09-16) | Same underscore-helper pattern, already live as `isEvaluated: False`; a promotion neither fixes nor touches it |
...
| Branch protection state | GitHub server-side (`gh api repos/spektrum-labs/Transformations/branches/main`) — **not in the repo** | Disabled as of 2026-09-16 (re-queried); the entire "gate" |
