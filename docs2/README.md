...
> Start here. Verified against `develop @ d3bece29` and production `main @ 6c0e6e63` (2026-09-16). Status: draft for engineer review.

**In one sentence:** This is the code-verified onboarding set for the Transformations repo — 803 per-vendor Python transform modules in production (867 on develop) that Token-Service fetches from GitHub raw and executes in a RestrictedPython sandbox during every evaluation, which makes a merge to `main` an instant, ungated production deploy — with a reading path for day one, your first transform, and everything after.
...
> [!IMPORTANT]
> **This repo's branch semantics are inverted relative to the sibling repos.** `main` IS production at fetch time — a merge to `main` deploys itself: typically live within seconds, worst case ≤300s of GitHub CDN plus ≤3600s of Token-Service's per-worker code cache, and rollback is another merge riding the same windows. `develop` is the staging ground: 564 commits ahead and invisible to production (a minted URL for develop-only code 404s), while `main` carries 93 hotfix commits never back-merged — including, since 2026-09-16, a 26-transform Tenable ASM vendor that is sandbox-fatal in its entirety ([14-known-issues.md](14-known-issues.md)).
>
> These docs are therefore pinned twice: the machine-readable pin `develop @ d3bece29` (the drift checker reads exactly that phrase from this file) plus production `main @ 6c0e6e63`, and behavioral claims about production cite `main:` paths. Full story: [13-release-and-branches.md](13-release-and-branches.md).
...
>
> - To know what production runs, read `main` — never `develop`, which is missing main's changes on 100 files including eight never-back-ported production changes. Diff `origin/main`, not the months-stale local branches.
...
- **Citations.** Inline `path:line` relative to the repo root, meaning the develop pin `d3bece29` — e.g. `local_tester.py:274`. Evidence from production carries a `main:` prefix and means `main @ 6c0e6e63` (because main is production, production behavior always cites `main:`). Cross-repo evidence names the repo and is verified against that repo's docs2 corpus (e.g. Token-Service `src/utils/codeexecutor.py:661`).
