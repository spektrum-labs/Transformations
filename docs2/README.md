...
> Start here. Verified against `develop @ 8bf278fb` and production `main @ 9d0262aa` (2026-09-04). Status: draft for engineer review.

**In one sentence:** This is the code-verified onboarding set for the Transformations repo — 772 per-vendor Python transform modules in production (859 on develop) that Token-Service fetches from GitHub raw and executes in a RestrictedPython sandbox during every evaluation, which makes a merge to `main` an instant, ungated production deploy — with a reading path for day one, your first transform, and everything after.
...
> [!IMPORTANT]
> **This repo's branch semantics are inverted relative to the sibling repos.** `main` IS production at fetch time — a merge to `main` deploys itself: typically live within seconds, worst case ≤300s of GitHub CDN plus ≤3600s of Token-Service's per-worker code cache, and rollback is another merge riding the same windows. `develop` is the staging ground: 536 commits ahead and invisible to production (a minted URL for develop-only code 404s), while `main` carries 83 hotfix commits never back-merged.
>
> These docs are therefore pinned twice: the machine-readable pin `develop @ 8bf278fb` (the drift checker reads exactly that phrase from this file) plus production `main @ 9d0262aa`, and behavioral claims about production cite `main:` paths. Full story: [13-release-and-branches.md](13-release-and-branches.md).
...
>
> - To know what production runs, read `main` — never `develop`, which is missing main's changes on 72 files including six never-back-ported production hotfixes. Diff `origin/main`, not the months-stale local branches.
...
- **Citations.** Inline `path:line` relative to the repo root, meaning the develop pin `8bf278fb` — e.g. `local_tester.py:274`. Evidence from production carries a `main:` prefix and means `main @ 9d0262aa` (because main is production, production behavior always cites `main:`). Cross-repo evidence names the repo and is verified against that repo's docs2 corpus (e.g. Token-Service `src/utils/codeexecutor.py:661`).
